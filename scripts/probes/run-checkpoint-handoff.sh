...
#
#   scripts/probes/run-checkpoint-handoff.sh --backend claude --scope session
set -euo pipefail

cd "$(dirname "$0")/../.."
out=".cc/temp/checkpoint-handoff-bundle.mjs"
mkdir -p .cc/temp

bun build scripts/probes/checkpoint-handoff.ts \
