      ...
      executionContexts: [
        {
          ...
          acceptanceCriteria: [
            ...
            { id: "sweeping", statement: "The handler covers every call site." },
...
describe("graph-workflow planner docs (D4 R16.3)", () => {
  ...
  it.each(SKILL_DIRS)(
    ...
    (dir) => {
      ...
      expectDocuments(skill, planReviewFindingsCommand(".cc/temp/plan.json"), why);
  ...
  it.each(REVIEW_SKILL_DIRS)(
    ...
    (dir) => {
      ...
      expect(entry, "`cctl workflow review` has no help-registry entry").toBeDefined();
      for (const flag of entry?.flags ?? []) {
        ...
        for (const value of (flag.valuePlaceholder ?? "").split("|")) {
