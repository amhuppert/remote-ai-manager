...
- `plan_defect` — a blocking validator seat refused the CONTRACT rather than the work: no task in the reviewed context can remedy what it found. The one halt here that is not exhaustion of a budget — nothing was retried, no task was reopened, and no failure was charged.
...
- A `plan_defect` halt reaches repair differently from the rest: it is routed at **first detection**, not after a retry budget runs out, so the agent arrives with one typed finding (the contract in conflict, and why no task can remedy it) and the preserved candidate instead of a whole halted context to re-diagnose. Repaired resumes automatically; a decline, a failure, or `planRepair` disabled leaves the run halted with the diagnosis in the halt summary. It is capped like every other repairable halt, per context — a defect the plan cannot answer would otherwise re-trip on every resume.
