...
When a halt of type `circuit_breaker`, `max_iterations`, `loop_limit_reached`, or `ownership_violation` lands and the `planRepair` policy is enabled (the seeded default), the engine automatically runs a bounded, read-only **plan-repair agent**:
