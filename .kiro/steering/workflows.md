...
| --- | --- | --- | --- |
| `archived-legacy-decode.ts` and archived execution repository | Read-only, all-or-nothing decoding of pre-assignment archives with frozen historical defaults; mixed generations are refused | `graph-workflow-archived-executions-repo.legacy-decode.test.ts` | Every retained archive has an independently preserved readable representation and none requires the decoder |
| `migrate-legacy-execution.ts` and execution codec | Normalize singular active-context and flat lane state; pause recovered lease holders while preserving worktree identity | `migrate-legacy-execution.test.ts` | No supported stored input has those shapes; retain interrupted-run recovery independently |
...
| `execution-origin.ts` and execution codec | Derive absent origin from template seeds; non-authoritative seed filler supports older shared-database readers | `graph-workflow-execution-codec.test.ts` | Retire the origin floor when no pre-origin rows remain; retire seed filler separately when no supported older reader needs it and a coordinated persisted-schema change is made |
...
| `context-scheduler.ts`, `context-landing.ts`, `lane-readiness.ts` and execution target resolver | Migration-only lane-less worktree contexts use fan-in landing and persisted retry intent; current assigned lanes use landing and joins | `execution-target-resolver.test.ts`, `lane-readiness.test.ts`, `execution-loop.test.ts` | No resumable execution requires lane-less fan-in or its merge retry; retain current read-only/session handling independently |
| `execution-lifecycle-port.ts` | Optional approval-wait/abort observers may be uninterested; absent admission observer admits unclaimed definitions. Running/delivery callbacks and execution policy remain mandatory | `execution-lifecycle-port.test.ts`, `execution-route-handlers.test.ts` | Supported optional integration, not migration debt; remove defaults only if every integration must implement the callbacks |
