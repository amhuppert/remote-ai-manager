...
This specification covers integrating browser-based voice transcription into CC by extending the existing Voice2Text CLI tool into an HTTP server and adding audio capture with a voice button UI in CC's session detail page. The feature spans two workstreams: Voice2Text server extension and CC voice integration. The browser captures audio via MediaRecorder, sends it through a CC API proxy route, which forwards it to the Voice2Text HTTP server with project context for transcription and cleanup. The cleaned text is returned and populates the prompt textarea. Additionally, when the user initiates voice recording from a text input that already contains text, that existing text is automatically sent as additional context to the Voice2Text server so that the cleanup phase can produce output that flows naturally from the prior content.

## Requirements

### Requirement 1: Voice2Text HTTP Server Mode

**Objective:** As a developer, I want Voice2Text to run as an HTTP server, so that CC can send audio for transcription over HTTP instead of requiring CLI invocation.
...
### Requirement 4: CC Voice Transcription API Proxy

**Objective:** As a developer using CC remotely (e.g., via Tailscale), I want CC to proxy voice transcription requests to the Voice2Text server, so that the browser does not need direct access to the localhost-bound Voice2Text service.

#### Acceptance Criteria

1. When a `POST /api/voice/transcribe` request is received with a FormData body containing `audio` (file) and `projectName` (string), the CC API route shall forward the audio and resolved project path to the Voice2Text server and return the transcribed text.
2. If the `audio` field is missing from the request, the CC API route shall respond with 400 and an error message.
3. If the `projectName` field is missing from the request, the CC API route shall respond with 400 and an error message.
4. If the project name cannot be resolved to a valid project path, the CC API route shall respond with 404.
5. If the Voice2Text server is unreachable, the CC API route shall respond with 502 and an appropriate error message.
6. If the Voice2Text server returns an error, the CC API route shall forward the upstream status code and error message.
7. If the transcription request exceeds 60 seconds, the CC API route shall abort and respond with 504.
8. The CC API route shall read the Voice2Text server URL from the `VOICE_SERVER_URL` environment variable, defaulting to `http://localhost:7880`.

### Requirement 5: CC Voice Health Monitoring

**Objective:** As a developer, I want CC to check whether the Voice2Text server is available, so that the voice button only appears when the service is operational.

#### Acceptance Criteria

1. When a `GET /api/voice/health` request is received, the CC API route shall check the Voice2Text server's health endpoint and return `{ available: true }` or `{ available: false }`.
2. If the Voice2Text server does not respond within 3 seconds, the CC health route shall return `{ available: false }`.

### Requirement 6: CC Browser Audio Recording

**Objective:** As a developer, I want CC to capture audio from my microphone in the browser, so that I can dictate prompts using my voice.

#### Acceptance Criteria

1. When the user starts a voice recording, the CC recording hook shall request microphone access via `getUserMedia` and capture audio using MediaRecorder.
2. The CC recording hook shall prefer `audio/webm;codecs=opus` format, falling back to `audio/webm`, then the browser default.
3. When the user stops recording, the CC recording hook shall assemble the captured audio into a Blob, send it to the CC transcription API with the project name, and deliver the transcribed text via a callback.
4. While recording is active, the CC recording hook shall track elapsed time in seconds.
5. While recording has been active for the maximum duration (default 300 seconds), the CC recording hook shall automatically stop the recording.
6. When the component using the recording hook unmounts, the CC recording hook shall stop any active recording, clear all timers, and abort any in-flight transcription requests.
7. The CC recording hook shall check voice server availability on mount and re-check periodically (every 30 seconds).
8. If microphone access is denied or the voice server is unavailable, the CC recording hook shall report itself as unavailable.

### Requirement 7: CC Voice Record Button UI
...
### Requirement 8: CC Session Detail Page Integration
...
1. If microphone permission is denied, the CC recording hook shall report the error "Microphone access denied".
2. If the recording is shorter than 0.5 seconds, the CC recording hook shall report the error "Recording too short".
3. If the Voice2Text server returns an error, the CC API route shall forward the error message to the client for display.
4. If the transcription request times out, the CC shall display "Transcription timed out".
5. If the Voice2Text server becomes unreachable mid-request, the CC shall display "Voice server is not available".
...
1. When the user initiates voice recording and the associated text input contains existing text, the CC recording hook shall include that text as a `context` field in the transcription request FormData.
2. If the associated text input is empty when recording starts, the CC recording hook shall omit the `context` field from the transcription request.
3. When the CC transcription API proxy receives a `context` field in the request FormData, the CC API route shall forward it to the Voice2Text server as a `context` field.
