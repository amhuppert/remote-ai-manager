...
- [x] 3.2 Implement charter draft lifecycle and activation
  - Implement beginning a draft (scaffold for a first charter, the existing charter for a redraft), filling a draft from agent content, approving a draft (activate as a new version, supersede the prior active, assign the next version number, materialize the mirror), and rejecting a draft (discard, leave active unchanged).
...
- [x] 3.3 Implement decision proposal and approval evolution
  - Implement persisting a durable bulk proposal batch without logging it, resolving a batch per-decision (approve or reject-with-feedback), appending only approved decisions to the log, creating an auto-activating draft linked to the approved decisions, and routing an incorporation message (approved) or feedback message (rejected) back into the originating conversation.
  ...
  - TDD: only approved decisions are logged; rejected proposals change nothing; a resolved approval ends with an auto-activated version linked to its decision(s).
  - Observable: approving decisions appends them to the log and produces a new active version linked to them with no separate charter approval; rejecting with feedback leaves the charter unchanged and enqueues the feedback.
  - _Depends: 3.2_
  - _Boundary: SessionAlignmentService_
  - _Requirements: 5.1, 5.3, 5.4, 5.5, 5.6, 6.1, 6.2, 11.2, 11.3_
...
## Task 4 — Core: agent entry points (`/align` command and tools)

- [x] 4.1 Implement agent-facing alignment tools
  - Provide a charter-write tool (fills the open draft, deferring draft-vs-activate to service state) and a non-blocking bulk decision-proposal tool (persists a durable pending batch and returns immediately without blocking the runtime).
  - Register both tools only for attended normal-session conversations — excluded from project conversations and autonomous turns — and ensure decision-log entries can originate only from these agent-proposed, user-approved decisions (no manual decision-capture path).
  - TDD: the write tool fills the open draft; the proposal tool stores a pending batch and returns without blocking; neither tool is available on an optimistic/autonomous turn.
  - Observable: an agent call to the proposal tool persists a reviewable pending batch and the turn ends; the tools are absent from an autonomous session's toolset.
  - _Depends: 3.2, 3.3_
  - _Boundary: Alignment tools_
  - _Requirements: 3.6, 5.1, 5.5, 5.7, 12.2, 12.3_
...
- [x] 6.1 Implement the alignment REST API
  - Expose endpoints to read the aggregate alignment state (including the live injection preview), approve/reject a charter draft, resolve a decision-proposal batch (per-decision approve/reject-with-feedback), fetch a per-version diff, and roll back to a prior version — each backed by the service and validated against the domain schemas, with router shells re-exporting the handlers.
  - TDD: each endpoint validates input and returns the expected state transition; approve/resolve produce activations, reject leaves state unchanged.
  - Observable: approving a draft via the API returns the newly active version; resolving an approval returns counts and produces an auto-activated version; invalid bodies are rejected.
...
- [x] 7.1 (P) Build the session-header alignment indicator
  - Add a header chip showing the four states: no alignment (an affordance to add one), active with version, draft/update pending behind the approval gate, and stale when a conversation has not seen the active version; use the "Alignment" label.
...
- [x] 7.3 (P) Build the approve-charter and decision-approval components
  - Build the "Approve Charter" banner (approve/reject a draft) and the bulk decision-approval component (per-decision approve or reject-with-note, reusing the existing question/option/note UI shape) backed by durable proposal state.
  - Observable: the banner approves/rejects a draft and the decision component submits per-decision approvals and reject-with-feedback against the API.
  - _Depends: 6.2_
  - _Boundary: ApproveCharterBanner, DecisionApprovalPanel_
  - _Requirements: 4.1, 5.2_

- [x] 7.4 Wire alignment gates into the conversation panel
  - Surface the approve-charter banner when a draft is pending and the decision-approval component when a proposal batch is pending, replacing the retired focus-confirmation wiring.
  - Observable: a pending `/align` draft shows the approval banner and a pending proposal batch shows the decision-approval UI within the conversation.
  - _Depends: 7.3, 6.2_
...
- [x] 8.3 Verify the decision evolution flow end-to-end
  - Add an integration test through the real store and prompt queue covering propose → resolve(approve) → incorporation message → auto-activation with decision linkage and an emitted alignment-updated event, plus reject-with-feedback leaving the charter unchanged and only approved decisions appearing in the log.
  - Observable: the test confirms an approved decision auto-activates a linked version and is logged, a rejected one changes nothing, and an alignment-updated event is broadcast.
  - _Depends: 5.3, 6.1_
  - _Requirements: 5.3, 5.4, 5.5, 5.6, 6.1, 6.2, 9.5_

- [x] 8.4 Verify creation-mode and alignment UI paths
  - Add UI tests asserting the create-session modal exposes only `normal`/`optimistic`, that approving a charter updates the header chip to the active version, and that the decision-approval component handles bulk approve and reject-with-note.
  - Observable: the modal shows no `focus`, the chip transitions to "active vN" after approval, and the decision component round-trips bulk approvals/rejections.
  - _Depends: 7.4, 1.5_
