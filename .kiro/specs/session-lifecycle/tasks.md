...
- [ ] 1. Add unit tests for session name validation and branch name sanitization
- [ ] 1.1 (P) Test session name validation rules
...
- [ ] 1.2 (P) Test branch name sanitization logic
...
- [ ] 2. Add integration tests for session creation flow
- [ ] 2.1 Test session creation with worktree and state persistence
...
- [ ] 2.2 Test session uniqueness enforcement
...
- [ ] 2.3 Test worktree path conflict detection
...
- [ ] 3. Add integration tests for init script execution
- [ ] 3.1 Test init script runs when configured
...
- [ ] 3.2 (P) Test init script error scenarios
...
- [ ] 4. Add integration tests for rollback on failure
- [ ] 4.1 Test rollback cleans up worktree and branch
...
- [ ] 4.2 Test rollback falls back to filesystem removal
...
- [ ] 4.3 (P) Test no state persistence on creation failure
  - Simulate any creation step failure
  - Verify `writeState` is never called with session data after a failure
  - _Requirements: 6.5_

- [ ] 5. Add integration tests for session deletion
- [ ] 5.1 Test successful session deletion
...
- [ ] 5.2 (P) Test session deletion with missing worktree
...
- [ ] 5.3 (P) Test session deletion fallback to filesystem removal
  - Simulate `git worktree remove` failure
  - Verify fallback to recursive filesystem deletion
  - _Requirements: 8.2_

- [ ] 5.4 (P) Test session deletion error cases
