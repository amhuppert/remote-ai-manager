...
| 2. Named Lane Continuity | `src/lib/workflows/graph-workflow/workflow-continuity-service.ts`, graph lane schemas in `src/lib/workflows/schemas.ts`, conversation totals in `src/lib/workflows/conversation/machine.ts` | Partial | Graph workflow already has the strongest prototype of `Lane`, including backend refs, rotation flags, limit evaluation, and stale-session handling. The abstraction is still graph-specific and not reusable by focus mode, regular conversations, optimistic mode, or future collaboration mode. |
