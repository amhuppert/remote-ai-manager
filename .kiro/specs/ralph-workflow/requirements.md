...
1. The CSM shall register an `initialize_ralph_loop` in-process MCP tool on every standard conversation prompt execution via the Agent SDK `query()` options.
...
3. When Claude calls `initialize_ralph_loop`, the CSM shall create a new workflow in `"planning"` status with the provided objective, using the same workflow data shape as the existing `POST /workflow` endpoint (config defaults, empty fix plan, initial circuit breaker state).
4. When Claude calls `initialize_ralph_loop`, the CSM shall immediately dispatch plan generation fire-and-forget using the provided objective as the sole context source (no transcript passthrough from the originating conversation).
5. When Claude calls `initialize_ralph_loop`, the CSM shall return a success response to Claude indicating the workflow was created and plan generation has started, along with guidance that the user should visit the dedicated workflow page to review and confirm.
6. While the session already has a workflow, the CSM shall not register the `initialize_ralph_loop` tool on conversation prompts (the tool is only available when no workflow exists).
7. If `initialize_ralph_loop` is called but a workflow already exists (race condition), the CSM shall return an error response to Claude indicating a workflow is already active.
8. When the workflow is successfully created via the tool, the CSM shall broadcast an SSE event so the session overview UI updates to show the new workflow.
...
1. The CSM shall provide a dedicated workflow page at the route `/projects/[name]/[session]/workflow`.
...
3. When the user clicks the workflow card, the CSM shall navigate to the dedicated workflow page at `/projects/[name]/[session]/workflow`.
...
5. The CSM shall update the workflow card in real-time via SSE events (status changes, plan updates, iteration progress) without requiring page refresh.
6. The CSM shall remove the existing "Ralph Loop" button from the session overview page action bar.
...
1. The CSM shall remove the "Workflow" tab from the conversation page right panel tab bar.
2. The CSM shall remove the `ConnectedWorkflowPanel` rendering from the right panel body.
3. The CSM shall remove the `hasWorkflow` prop from the right panel component and all related prop threading from parent components.
4. The CSM shall remove `"workflow"` from the right panel tab type definition in the session detail store.
