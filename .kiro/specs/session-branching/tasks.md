...
- [x] 1. (P) Extend session and request schemas with branching fields
...
- [x] 3.2 Implement orphan session retargeting when parent is merged or deleted
...
- [x] 4.2 Thread target branch through merge and squash merge actors
...
- [x] 4.3 Include target branch in background job dispatch and notification messages
...
- [x] 5. Integrate session branching into API routes and UI
- [x] 5.1 Update session creation route with parent session validation
...
- [x] 5.2 (P) Update merge, resolve-conflicts, commit, and diff routes to use session's target branch
...
- [x] 5.3 Create BranchSelector component for parent-session selection
...
- [x] 5.4 Add Target column and Branch action to SessionsTable
...
- [x] 5.5 (P) Add targetBranch prop to SmartMergeDialog and MergeToast
...
- [x] 5.6 (P) Replace remaining hardcoded "main" references in UI components
