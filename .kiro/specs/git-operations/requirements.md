...
**Objective:** As a developer, I want to commit all changes in a session's worktree from the CSM UI, so that I can create meaningful checkpoints without switching to a terminal.

#### Acceptance Criteria

1. When the user initiates a commit action from the session detail page, CSM shall display a commit dialog requesting a commit message.
2. When the user submits the commit dialog with a non-empty message, CSM shall stage all changes (modified, new, and deleted files) and create a commit in the session's worktree branch.
3. When the commit succeeds, CSM shall dismiss the dialog, refresh the session view to reflect the new commit, and update the diff panel (since committed changes are no longer uncommitted).
4. If the worktree has no uncommitted changes when the user initiates a commit, CSM shall disable the commit action and indicate that there are no changes to commit.
5. If the commit operation fails (e.g., git error), CSM shall display the error message within the commit dialog without dismissing it, allowing the user to retry or cancel.
6. While a prompt is running (session status is "running"), CSM shall disable the commit action to prevent committing mid-execution.
7. While the session is finished, CSM shall disable the commit action.

### Requirement 2: Squash Merge Session Branch into Main

**Objective:** As a developer, I want to squash-merge a session's branch into `main` from the CSM UI, so that I can integrate completed work as a single clean commit without leaving the dashboard.

#### Acceptance Criteria

1. When the user initiates a merge action from the session detail page, CSM shall display a merge dialog showing the branch name, total number of commits to be squashed, and a field for the merge commit message.
2. When the user submits the merge dialog, CSM shall perform a squash merge of the session branch into `main` using the provided commit message.
3. When the squash merge succeeds, CSM shall mark the session as finished, archive the session, and navigate back to the project's sessions list.
4. If the session's worktree has uncommitted changes when the user initiates a merge, CSM shall block the merge action and display a message indicating that uncommitted changes must be committed or discarded before merging.
5. If the squash merge fails due to conflicts or other git errors, CSM shall display the error message within the merge dialog without dismissing it, allowing the user to cancel and resolve issues manually.
6. While a prompt is running (session status is "running"), CSM shall disable the merge action to prevent merging mid-execution.
7. If the session branch has no commits beyond `main` (nothing to merge), CSM shall disable the merge action and indicate that there are no changes to merge.
8. While the session is finished, CSM shall disable the merge action.
...
2. When the user archives a session, CSM shall mark the session as archived and it shall no longer appear in the default sessions view.
3. When the user unarchives a session, CSM shall remove the archived flag and the session shall reappear in the default sessions view.
...
1. When a session is marked as finished (via successful merge in Requirement 2), CSM shall persist the finished state and the session shall become read-only.
