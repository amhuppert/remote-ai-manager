...
## Introduction
This specification covers the focus session initialization flow improvement for CC (Command Center). The goal is to transform focus mode session creation from a single monolithic prompt into a structured multi-step flow with explicit user confirmation. The initialization conversation — where Claude researches the objective and asks clarifying questions — is treated as a distinct phase. A confirmation bar lets the user signal satisfaction, after which the focus document is written, the initialization conversation is archived, and the user is navigated to a fresh regular conversation.
...
#### Acceptance Criteria
1. When a focus session is created, CC shall mark the first conversation with a role of "initialization".
2. When a non-focus (fast) session is created, CC shall set the conversation role to null.
...
#### Acceptance Criteria
1. When a focus session's initialization conversation starts, CC shall send only the "understand objective" prompt (research, ask questions, present summary).
2. While the understand-objective prompt is executing, CC shall not write the focus.md document.
...
#### Acceptance Criteria
1. When the active conversation has role "initialization" and the conversation is not busy and has at least one completed prompt, CC shall display a confirmation bar.
2. While a blocking question (AskUserQuestion) is pending, CC shall hide the confirmation bar.
3. The confirmation bar shall display the text "Satisfied with the understanding?" and a "Confirm & Continue" button.
4. While the session is in a finished state, CC shall disable the confirmation button.
...
#### Acceptance Criteria
1. When the user clicks "Confirm & Continue", CC shall send the "write focus document" prompt to the initialization conversation.
...
#### Acceptance Criteria
1. When the focus document has been written successfully, CC shall archive the initialization conversation (set archived to true).
2. When the initialization conversation is archived, CC shall not display it in the conversation sidebar unless archived conversations are shown.
...
#### Acceptance Criteria
1. When the initialization conversation is archived, CC shall create a new conversation with role null.
2. When the new conversation is created, CC shall navigate the user to it automatically.
