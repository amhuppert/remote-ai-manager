...
> **Reading note (2026-09-24).** Revisions and delivery-plan attempts no longer have a Proposed state. A draft is reviewed while it stays editable, and sign-off is the only freeze; Requirement 27 is authoritative for that lifecycle. Criteria marked **Superseded by 27.n** describe retired behavior and are kept only as history.
...
17. The spec system shall treat an element's parent as part of its stable identity: an ordinary update that names a parent different from the stored parent shall be refused with a typed refusal stating the rule, its rationale, and the replacement path (create under the desired parent, remove the old element from the draft) — never accepted with the parent silently preserved. Echoing the stored parent or omitting it shall remain legal.
...
#### Acceptance Criteria
1. **Superseded by 27.2.** The spec system shall derive the spec phase — Draft, In review, Approved, Executing, Delivered, Abandoned — from revision and execution state; the phase shall never be directly settable.
2. **Superseded by 27.2.** The spec system shall report Draft while the current revision is editable and not yet proposed, In review while a proposed revision awaits approvals, Approved while an approved revision exists and no execution is active, and Executing while an active execution runs against a pinned revision.
3. **Superseded by 27.1.** The spec system shall move each revision only through Draft → Proposed → Approved or Withdrawn; when a draft revision is proposed, the spec system shall freeze its content for review.
...
3. The `cctl spec` family shall provide reads for: spec inventory; bounded outline, summary, canonical rendered, and full artifact views; spec status (phase, the current authoring stage and its concluding gate, gate states, pending approvals, active attention, and coverage); single-element retrieval with approval, evidence, attention-record version, lifecycle, provenance, lineage, human capability, and current-draft citation state where applicable; single-section retrieval by stable element id; explicit historical revision selection for element and section reads; bounded current/history attention projections; and text search over requirements and decisions.
4. The `cctl spec` family shall provide writes for: draft-revision content updates, proposing a draft for review, advancing the authoring stage where the governing dial admits it, opening questions, proposing assumptions, agent correction of unresolved attention records, append-only supersession of disposed assumptions, draft citation changes, human answers and dispositions, evidence-backed task completion claims, and routing approval requests to the human.
...
10. An element read that names no revision shall answer only from the current revision. When the addressed handle exists only in historical revisions, the read shall refuse with a typed `historical_only` response naming the last containing revision and the exact explicit historical read command, never silently answering from an older revision; an explicit revision selector (by revision number or id) shall serve intentional archaeology; and spec-scoped question/assumption record reads shall be unaffected.
11. The spec system shall provide a narrow read of exactly one current-revision section by its stable element id — returning an explicit null handle, role, title, body, element version, and position — without introducing a section handle vocabulary; sections shall be discoverable from the bounded outline with their element ids; and historical section reads shall use the same explicit revision selector as element reads.
...
4. **Superseded by 27.7.** While a proposed revision is under review, Spec Studio shall present a semantic change list — element-level add/remove/modify with kind-aware summaries — with each change deep-linked to the affected element and carrying inline approve/comment controls; a raw diff shall remain available as a secondary view.
5. **Superseded by 27.7.** Spec Studio shall offer exactly these explicit review actions: Comment (never unfreezes the proposed revision), Request changes (ends the review attempt and opens a draft revision), Approve item, and Sign off revision.
...
11. Spec Studio shall present questions and assumptions as an attention register with current questions and assumptions first and withdrawn records and superseded predecessors in a separate record-history disclosure; answered questions and human-disposed assumptions shall remain current durable facts but shall not count as active attention.
12. Each attention-record card shall distinguish creation provenance from the latest typed mutation provenance, expose stable handle deep links and display attachment separately from revision citations, and render only server-derived human capabilities rather than reimplementing lifecycle, frozen-citation, amendment, or abandoned-spec rules in React.
13. Only an open question shall offer one versioned human answer act, and only a proposed assumption whose server-derived capability allows disposal shall offer one explicit Confirm, Reject, or Defer act; answered, disposed, withdrawn, and superseded records shall expose no terminal mutation controls.
14. Attention-record mutation state shall be card-local: pending, failure, and success on one record shall not disable siblings; a failed submission shall preserve authored input and announce the error; a successful submission shall restore focus to the mutated record.
15. The Review surface shall render assumption premises from the selected revision's frozen citation snapshots rather than from mutable current assumption rows, and the History surface shall render typed durable attention-record and citation events without synthesizing actors or operations from current-row timestamps.
16. At a 390 CSS-pixel viewport and 200% browser zoom, the attention register shall reflow without horizontal scrolling, preserve document order and readable metadata/action wrapping, expose at least 44-by-44 CSS-pixel targets, and remain operable by keyboard and screen reader.
...
8. While a revision-owned citation snapshot records a rejected assumption, the spec system shall raise a blocking finding that prevents revision sign-off (the sign-off precondition in Requirement 10); lint shall never infer citation authority from a mutable assumption display attachment.
9. The spec system shall raise advisory, non-blocking findings for: an approved element changed in the current draft (approval freshness — feeding invalidation at propose per Requirement 10); a change to an element cited by an approved element (dependency change — the citing element's approval remains valid in V1); active open questions at propose; and a materialized task removed or re-scoped by an amendment (Requirement 15).
...
14. When a draft revision is proposed while any Markdown prose field of the current snapshot — section titles and bodies, requirement statements, criterion text and validation notes, decision titles, chosen approaches, reasons, and rejected-alternative reasons, and task titles and instructions — contains a handle-shaped token at a lexical boundary that resolves to no current element or to no existing question/assumption record, the spec system shall refuse the proposal with a dangling-handle finding naming the source element, the field, and the token, distinguishing removed from unknown targets. Fenced code blocks, inline code spans, autolinks, raw URLs, and link destinations shall not be scanned — masked code is the documented escape for literal tokens — and question/assumption references shall validate on record existence regardless of lifecycle state. The false-positive boundary shall be pinned by tests; foreign-spec slug-qualified tokens shall be recognized but not validated.
...
4. The spec system shall permit the revision sign-off transition only when all blocking comment threads are resolved and no frozen revision citation snapshot records a rejected assumption; where the governing dials require human approval, sign-off shall additionally require the configured element approvals to be in place.
5. **Superseded by 27.3.** When a draft is proposed, the spec system shall classify every element as added, unchanged, modified, or removed; added elements shall remain visibly distinct from edits, approvals on unchanged elements shall carry forward automatically, approvals on modified elements shall become stale with the element-level change shown side by side for re-approval, and approvals on removed elements shall be closed.
...
8. The spec system shall invalidate approvals on direct content change and on a change to the revision-owned assumption citations covered by the approval subject: requirement approval shall fingerprint the requirement, its criteria, and their citation subhash; decision approval shall fingerprint its citation subhash; revision sign-off shall fingerprint the full citation hash. A change to an ordinary element reference remains direct-change-only and raises the dependency-change advisory finding (Requirement 9) instead.
...
12. When a draft revision is successfully proposed, the spec system shall itself file the gate-scoped approval request for every authoring gate the proposal leaves pending — idempotently, reusing the existing request identity and stable attention id — and the proposal receipt shall report a typed per-gate outcome: `filed`, `already-filed`, `not-needed`, `delivery-uncertain` (durable request committed, notification uncertain), or `not-filed` (filing failed before durable commit). Neither a filing failure nor a notification failure shall fail or roll back the proposal, and the explicit request-approval act shall remain the idempotent recovery verb.
13. A notification failure after a durable approval-request commit shall degrade to a typed delivery outcome on the acting receipt — never an operation failure for an act that durably succeeded — and re-invoking the request act shall re-ensure the notification from the stable attention id without creating a duplicate request.
...
**Objective:** As the operator, I want questions and assumptions to be correction-safe, auditable records whose cited premises are frozen with each revision, so that unresolved intent stays visible without rewriting human judgment or historical review truth.

#### Acceptance Criteria
1. The spec system shall record questions as stable, versioned, addressable records carrying text, a spec-or-element display attachment, creation provenance, and exactly one lifecycle state: `open`, human-terminal `answered`, or agent-terminal `withdrawn`.
2. The spec system shall record assumptions as stable, versioned, addressable records carrying text, a spec-or-element display attachment, creation provenance, lineage, and exactly one disposition: `proposed`, human-terminal `confirmed`, `rejected`, or `deferred`, or agent-terminal `withdrawn`.
3. Every question and assumption mutation shall enforce cross-field consistency between lifecycle, answer/disposition data, and terminal timestamps in both application validation and database constraints.
4. Any authenticated authoring agent shall be allowed to edit or withdraw an unresolved attention record regardless of which conversation created it; creation conversation shall remain provenance and shall never become authorization.
5. Only a human actor shall be allowed to answer an open question or dispose a proposed assumption, and only an authenticated authoring agent shall be allowed to edit or withdraw an unresolved record, supersede a disposed assumption, or cite or uncite an assumption; each authority check shall be enforced inside the application service in addition to route admission.
6. Answered questions, human-disposed assumptions, withdrawn records, their creation identity, and their human terminal result shall be immutable; a second answer or disposition, including an identical retry, shall be refused while the first receipt remains readable.
7. Correcting a confirmed, rejected, or deferred assumption shall append one new proposed successor with a new stable `An` handle and an immutable same-spec predecessor edge; it shall not alter the predecessor's disposition. A successor operation shall be idempotent by operation id plus canonical request hash and shall refuse conflicting reuse.
8. Every attention record shall carry a compare-and-swap record version beginning at one; every legal row mutation shall require the expected version and legal source lifecycle and shall increment the version exactly once, while a stale writer shall receive a typed refusal and a bounded recovery read.
9. Every revision shall own an explicit set of assumption citations keyed by revision, subject element, and assumption, and every citation shall freeze a strict versioned snapshot of the cited premise, including its text, lifecycle, record version, provenance, display attachment, lineage predecessor, and capture provenance.
10. An assumption's display attachment shall be presentation metadata only. Changing it shall never implicitly create, move, or remove a revision citation; an agent edit that changes attachment shall provide an explicit preserve-or-replace citation intent.
11. **Superseded by 27.1.** Citation rows, citation snapshots, citation version, citation contract version, and citation hash shall be mutable only while their owning revision is a draft. Proposed, approved, and withdrawn revisions shall remain frozen through service and repository guards backed by database triggers.
12. Every draft revision shall carry a compare-and-swap citation version beginning at one and a deterministic citation hash over the citation contract version and sorted citation triples plus snapshots. A transaction that changes a citation triple or refreshes a cited snapshot shall require the expected citation version, increment it exactly once, and update the hash; a no-op shall not increment it.
13. Editing a cited proposed assumption's text shall refresh every citation snapshot for it in the current draft in the same transaction; withdrawing one shall remove every current-draft citation to it in the same transaction. If frozen history cites that assumption, a writable amendment draft shall exist first, and no frozen snapshot shall change.
14. The first human disposition of a proposed assumption shall remain legal. If the current draft cites it, that transaction shall refresh the current-draft snapshots using citation compare-and-swap; if frozen history cites it and no writable amendment exists, the service shall refuse with `amendment_required` and the exact recovery act.
15. Superseding a disposed assumption shall require a current amendment draft, predecessor record version, draft citation version, reason, successor text and attachment, and an exact clear-or-replace citation intent. The transaction shall create the successor, make the predecessor inactive, replace or clear current-draft citations, update versions and hashes, append audit events, and publish invalidation only after commit.
16. Proposed, confirmed, and deferred assumptions may gain a draft citation. Rejected, withdrawn, and superseded assumptions may not gain one; removing a citation shall not alter the assumption's display attachment.
17. **Superseded by 27.7.** New empty drafts, ordinary and amendment forks, Request Changes, attached-assumption creation, element removal, proposal, and born-approved import shall each own an explicit atomic citation rule: initialize, copy, create, cascade-remove, verify-and-freeze, or materialize-before-finalize as applicable; reintroducing an element shall never restore citations implicitly.
18. The spec system shall expose three complete projections: current records exclude withdrawn rows and superseded predecessors; active attention contains only current open questions and current proposed assumptions; record history contains withdrawn rows and superseded predecessors. Detail lookup, typed History, lineage links, and canonical export shall retain every record.
19. Active attention shall not itself block sign-off. Status, Needs You, and badges shall count active attention only, while the canonical server lint finding and its `blocks_signoff` flag shall remain the sole sign-off authority.
20. Every attention detail shall expose server-derived current/history and active state, creation and latest typed-mutation provenance, predecessor and successor handles, and a human capability that states whether answer or disposition is allowed or refused as terminal, amendment-required, or read-only with its recovery instruction.
21. Every attention-record mutation shall append a strict before/after `spec-review-record-mutated` audit event, and every citation-set mutation shall append a strict `spec-assumption-citations-mutated` event, in the same transaction as the durable change. Operational logs and default receipts shall contain ids, operations, versions, counts, and refusal codes but shall not echo question, assumption, answer, or reason bodies.
22. Revision semantic diff, lint, approval applicability, proposal, import, canonical export, and integrity verification shall consume revision-owned citation snapshots and the full `(content hash, citation contract version, citation hash)` integrity tuple; a citation-only change shall never appear unchanged, and verification shall detect citation or snapshot tampering and same-spec or revision-membership violations.
23. Existing frozen revisions shall cross the coordinated schema cutover under a legacy citation contract that records exact cutover state without claiming citation-aware human approval; existing drafts and every new revision shall use the citation-aware contract. Legacy element approval shall carry across the first boundary only when the subject payload is unchanged and neither side has citations; revision sign-off shall never carry.
24. Canonical export shall include complete versioned question and assumption rows, sorted revision citations and strict snapshots, citation integrity metadata, and only the typed attention-record and citation audit event families. Canonical bundle consumption shall be a read-only format-4 decode-and-verify boundary: before any network request or durable write, it shall reject a missing or different format with `bundle_format_mismatch`, strictly validate the complete format-4 structure and semantics, canonically re-render the manifest and Markdown, and compare the manifest bytes and ordered Markdown paths and content with precise, non-body integrity diagnostics. It shall provide no persistent canonical restore/import path, old-bundle reader, conversion, dual write, shape inference, or fallback. `cctl spec import` shall remain the separate external-source authoring import and shall reject canonical export bundles.
25. The `cctl spec attention` family shall provide strict, versioned `edit`, `withdraw`, `supersede`, `cite`, and `uncite` acts with file-backed complex payloads, read preflight for required compare-and-swap tokens, typed refusal recovery, non-echoing receipts, and exact follow-up reads; guidance shall direct obsolete records to correction commands and shall never instruct an agent to reject an assumption or park premature design in Q/A.
26. Creation, correction, answer, disposition, supersession, cite, and uncite shall all refuse an abandoned spec. The schema cutover shall rebuild the authoritative question and assumption tables, materialize exact legacy citations, verify every relation and hash before commit, install frozen-revision triggers, bump the forward-only schema barrier, and provide no compatibility shim, fallback inference, mixed-version operation, or old-bundle reader.
...
4. Attention invalidation shall distinguish active and inactive records so withdrawn and superseded records leave active counts while remaining available in current/history reads and typed audit history; mutation publication shall occur only after the durable transaction commits.
...
7. **Superseded by 27.9.** A draft opened from an approved revision shall open at the stage after its base (capped at plan); a draft opened by request-changes shall retain the withdrawn revision's stage.
...
19. **Superseded by 27.8.** Status, proposal, reopen, and withdrawal outputs shall report both sides of the approval ledger — subjects satisfied, classified as carried from an ancestor revision, approved on the current revision, import-settled, or covered by a recorded combined act, and subjects pending — with the carry rule stated in one line. Classification shall derive from the same approval-applicability authority the gates consult. A collapsed fast-path gate shall report its subjects as governed by the combined act — before sign-off as outstanding through it, after sign-off as satisfied by it — never as not-applicable and never as a manufactured zero-count ledger; a policy-admitted (Notify/Off) gate shall be reported at gate level as an admission, never as per-subject approvals.
20. **Superseded by 27.8.** Typed refusals that enforce a designed constraint shall carry a one-sentence server-authored rationale, rendered as a distinct `why:` line between the unmet condition and the instruction — present at minimum on the stage-blocked, human-act-required, withdraw-after-engagement, and parent-immutability refusals — asserting the constraint's value without apology; rationale shall never render on success paths.
21. **Superseded by 27.8.** Agent guidance shall classify designed constraints (staged authoring, human-only acts, withdraw-after-engagement, frozen revisions, stable handles, immutable parents, machine-provable validation strategies) as product behavior to lean into, distinct from incidental defects to report; shall teach batch-local element-id cross-referencing for structured references created in one batch; shall document the masked-code escape for literal handle tokens in prose; and shall state that proposing files approval requests automatically with request-approval as recovery.
...
2. **Superseded by 27.9.** Extending an approved spec shall begin with a Requirements draft for the extension, then open a separate Design draft only after the Requirements checkpoint settles.
3. **Superseded by 27.9.** Returning from Design to Requirements shall withdraw the Design attempt and base the new Requirements draft on the latest approved Requirements checkpoint, excluding unapproved Design edits.
...
5. **Superseded by 27.10.** A proposal shall freeze the exact definition id, definition revision, definition hash, binding hash, and candidate hash. Reopening shall clone the frozen definition and preserve the prior candidate.
...
### Requirement 27: Continuous review of the editable draft

**Objective:** As the operator, I want to review and approve whenever a spec or delivery plan carries unapproved changes, without waiting for the author to freeze a proposal, so that there is one model: unapproved changes are reviewable, and only sign-off freezes. (command-center#160, 2026-09-24. Criteria marked "Superseded by 27.n" earlier in this document describe the retired Draft → Proposed → Approved lifecycle.)

#### Acceptance Criteria

1. A spec revision shall move only through Draft → Approved or Withdrawn; there shall be no Proposed state. The draft, including its assumption citations, shall stay editable while it is reviewed, and revision sign-off shall be the only transition that freezes revision content.
2. The spec phase shall report Draft while the current revision is editable, whether or not review has been requested, and shall have no In review phase. Pending review shall be visible through the draft's unapproved subjects and open approval requests.
3. A human may approve a requirement or decision on the draft at any time. Each content approval shall record the fingerprint of the subject it approved and shall apply only while the subject's current fingerprint matches it, so editing an approved subject makes it unapproved again. Approvals carry from approved ancestors under the same rule, and the draft is classified against its nearest approved ancestor.
4. Every human review act — approving an item, bulk approval, and sign-off — shall state the review hash of the draft content the human read. When the draft has changed since, the act shall be refused as `stale_review` and nothing shall be recorded.
5. `cctl spec propose` shall request review: it shall run the propose lint gate, file the approval request for every gate that needs a human, and leave the draft editable. Where every consulted gate is Notify or Off, propose shall itself freeze and approve the draft, record the policy admissions, and retire any open approval requests for it.
6. Sign-off shall require every consulted Gate subject to be approved on the current content, no blocking lint findings, and no unresolved blocking comment threads. It shall compute the content hash, freeze the revision, and record its admissions in one transaction.
7. Humans shall comment directly on the draft; a blocking comment takes the place of Request Changes, which shall not exist. Spec Studio shall present the semantic change list against the nearest approved ancestor, with inline approve and comment controls, whenever the draft carries unapproved changes. Review feedback shall route to the draft's author, the latest agent that changed the revision.
8. Withdrawing an open draft shall be a human discard. Agent withdraw-proposal, withdraw-after-engagement, request changes, dismissing superseded proposals, and proposal supersession shall not exist; status and receipts shall report the approval ledger of the current draft.
9. An amendment of an approved spec shall open at Design, and requirements it leaves unchanged keep their approvals. An amendment that changes a requirement or criterion shall first return to Requirements: the Design draft is withdrawn, and a Requirements draft opens from the revision it amended, keeping the approved Design and excluding the withdrawn draft's Design edits. After the Requirements sign-off, a further amendment opens Design again.
10. A delivery-plan attempt shall move through Draft → Approved → Parked or Launched, or to Abandoned, with no Proposed state. Its managed definition and binding shall stay editable while reviewed. The human signs the draft off in Workflow Builder, stating the binding and definition revisions they reviewed; that sign-off shall freeze the exact definition id, definition revision, definition hash, binding hash, and candidate hash, and admit execution start, in one transaction. `cctl spec plan propose` shall request that review, or, under a Notify or Off execution-start dial, freeze and approve the draft itself. Parking shall require an approved attempt, and reopening shall apply to approved or parked attempts.
11. Persisted state from the retired lifecycle shall migrate once: a proposed revision reopens as its spec's draft, or is withdrawn when the spec already has one; existing content approvals gain the fingerprint of the revision they were granted on; proposal supersession records are dropped; and proposed attempts, or attempts parked without a sign-off, are abandoned.
