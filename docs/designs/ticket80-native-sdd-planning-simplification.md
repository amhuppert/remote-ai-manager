...
Status: PROPOSED, revision 1, 2026-09-03. Design only. The one change already shipped under this ticket is the outcomes-only validator rule (section 3.9), which is independent of everything else here.

Scope: `src/lib/specs/{delivery-plan,delivery-plan-seed,delivery-plan-finalization,delivery-plan-service,delivery-plan-binding-lint,execution-claims-document,export}.ts`, `src/lib/workflows/{definition-route-handlers,committed-source-locator-lint,plan-validation}.ts`, `src/lib/workflows/plan-review/schemas.ts`, `src/lib/workflow-graph/{locked-regions,criteria/criterion-records,charter/render,shared-documents,validation,runtime-edits,execution-loop}.ts`, `src/cli/commands/spec/write.ts`, `src/cli/commands/workflow-outline.ts`, the `native-sdd-authoring` and `graph-workflow-planning` skills, and `.claude/commands/spec.md`.

Relationship to prior designs: this builds on `docs/design/2026-08-14_direct-authored-delivery-plans.md` (decisions 1 to 6, settled 2026-08-14) and product design §5.13 (approved 2026-08-31). It keeps decision 1 (blocking delivery gate), decision 3 (must-run coverage lock), decision 4 (invariant scoping) and decision 6 (live amend retired). It amends decision 2: claims stop being an authored sidecar and become a derivation from a per-criterion `covers` annotation on the graph definition. Section 8 lists the decisions Alex needs to settle.
...
3. **One id and one labelled token per object.** A receipt that prints an id says which verb takes it; a receipt that prints a revision says which write it guards.
4. **Designed friction states its reason at the point of refusal.** Sign-off, the candidate freeze, human dispositions and the must-run lock stay. Each refusal carries a `why:` line.
5. **Generalize before special-casing.** Anything the SDD path needs that ordinary plans could use (seeded documents, scoped excerpts, id-bearing locators) lands on the shared engine, per the standing rule that graph workflow features stay general.
...
- The refusal instruction becomes stage-aware: on a draft, "this field is server-owned; omit it from your plan"; on a candidate, the existing "reopen and re-propose".
...
`structuralIssuePath` in `plan-validation.ts` appends the record id in parentheses after any indexed segment whose record carries an `id`: `definition.tasks.2 (wire-routes).contextId`, `definition.executionContexts.9 (memory-cli).scriptValidator.commands.3`, `definition.charter.sourcesOfTruth.1 (seeded-doc).locator`. The JSON path stays first for machine consumers; the `--json` envelope gains `recordId` beside `path`. Lint warnings use the same formatter. This is shared-engine work and benefits every plan.

### 3.3 Coverage replaces authored claims (I-9, I-16 in part)
...
`covers` is an optional array of external ids. The graph engine treats it as opaque traceability; it renders `(covers: memory-crit-status-withheld)` beside the record in implementer and validator prompts and carries it into verdict citations. The spec layer interprets it.

Claims become a derivation: context C claims spec criterion S when any criterion of C covers S and S is selected. `spec plan propose` computes the claims and freezes them in the candidate manifest, so a frozen candidate remains self-describing and the delivery gate reads the same shape it reads today. The binding document a planner writes shrinks to dispositions, which are human-side already, so on the ordinary path `spec plan edit` is never needed: `spec plan open` seeds dispositions from the delivery delta, the planner authors the graph with `covers`, and propose does the rest.

Lint moves from the binding to the definition, keyed by criterion id:

| today | after |
|---|---|
| `binding/selected-criterion-unclaimed` | `coverage/selected-criterion-uncovered`: no criterion in a stable authored context covers it |
| `binding/selected-criterion-not-must-run` | unchanged rule, fed by derived claims; the message names the covering criteria and suggests the attester idiom (3.6) |
| `binding/claim-criterion-unknown` | `coverage/unknown-id`: `covers` names an id the pinned revision does not carry |
| `binding/claim-criterion-unselected` | `coverage/unselected`: covers a deferred or waived criterion (still an error; covering out-of-scope work is a contradiction) |
| `binding/claim-context-unstable` | `coverage/unstable-context`: the covering context is dynamic (loop clone, expansion child); the authored ancestor must cover it |

Evidence grain improves without a policy change: the delivery gate today attributes a context's validator GO to every criterion the context claims. With `covers`, a verdict that cites `criterionId` attributes to exactly the spec criteria that record covers. Decision 1 of 2026-08-14 (gate blocking) is unchanged; its grain moves from context to criterion.

Planner guidance, written into both skills: spec criteria are the contract, context criteria are the validator's checklist written in observable terms, and `covers` is the link. Never paste spec criterion text into a context criterion. A context criterion may cover none (plan-level wiring and cross-context obligations), several may cover one, and one may cover several.

Migration: binding schema version 4 carries `dispositions` only; the candidate manifest carries derived `claims`. Draft and proposed v3 attempts at cutover reopen and re-propose (they are pre-approval by definition, the same policy the v3 cutover used). Launched v3 attempts stay readable; the gate reads claims from the manifest for both versions. `spec plan edit` refuses a `claims` key with a message pointing at `covers`.
...
- **One execution id.** The `spec start` receipt prints `execution: <workflow execution id>` and labels it as the id `workflow status`, `workflow wait` and `workflow live` take. The spec-side execution row id stops appearing in agent-facing text; `spec status <slug>` and `spec abandon <slug> --execution <id>` accept the workflow execution id, which the link table already resolves.
...
- **Abandon retires the attempt.** The abandon coordinator's finalize phase records the `abandon` transition on a launched attempt, so `nextAct` reads `spec plan open` and the `spec abandon` receipt prints it. `spec start` on an attempt whose execution was abandoned then refuses with that same exit instead of "not signed off".
- **`requires-pause` states its reason:** "structural edits apply atomically against a quiescent scheduler, so no lane reads a half-applied definition; pause, edit, resume".
- **Seed charter and a propose check.** `spec plan open` seeds the mission from the spec's intent section and no `delivery-plan-authoring` source; propose refuses `launch/charter-unauthored` (blocks propose) when the mission still equals the seed text or the charter cites no authored source. The stub that #98 froze cannot recur.
- **Outline hints.** `cctl workflow get` prints the selector beside each size line (`charter: mission 646 chars · 12 sources → --charter`). `spec plan preview` gains `--outline`, delegating to the workflow outline renderer.

### 3.6 Guidance routing (I-8, I-17, I-19, I-20)

- `.claude/commands/spec.md` and the native-sdd skill: after design sign-off, `spec plan open`, then author the graph with the `graph-workflow-planning` skill as an ordinary `plan.json` with `covers`, `cctl workflow validate`, `cctl workflow replace <definitionId>`, `spec plan propose`, human sign-off, `spec start`. Workflow Builder is named once, as the human's review surface.
- The planning skill gains a short section, "Delivering a native spec": the two spec-specific concepts (`covers`, pinned revision); the attester idiom for disjunctive delivery, stated with its reason (a claimed criterion must be covered on every path so a skipped branch can never waive it silently; when two guarded routes each deliver it, also cover it from an always-run closeout context that verifies whichever route ran); phase-scoped authoring (contexts, criteria, placement and edges validate before any task exists); the launch sequence and the post-abandon exit; edge ids and the endpoint-pair removal fallback.
- Until I-11 is fixed, the skill carries the interim rule: pause only after `workflow status` shows a lane active.
...
| Constraint | Protects | Legibility change |
|---|---|---|
| Human dispositions (deferred, waived, reaffirmed) | nothing leaves scope silently | none; already Studio-owned |
| Candidate freeze and sign-off by hash | the human approves exact bytes | the sign-off receipt shows the plan review verdict (3.7) |
| Pinned revision | validators judge the contract the run launched against | none |
| Must-run coverage lock | a skipped branch can never waive a claimed criterion | the lint message names the attester idiom; the skill documents it (3.6) |
| `requires-pause` for structural live edits | no lane reads a half-applied definition | the refusal states the reason (3.5) |
| `/origin` and `/approvalRequired` locks on a draft | provenance | replace merges around them (3.1) |
...
- The hash change in 3.7 alters no ordinary definition's hash, and no review recorded on a managed definition exists to invalidate.
- The `spec start` receipt change removes a token agents were told to ignore; no documented command consumes the spec-side id.

## 6. Tests

Each area starts from a failing behaviour test in the module's existing test file.

- 3.1: `definition-route-handlers` contract tests: a bare `plan.json` replaces a managed draft and the stored record keeps origin, approval policy and locks; a plan with a different `origin` is refused naming `/origin` with the draft instruction; a candidate is refused with the reopen instruction; a reopen preserves authored edge ids.
- 3.2: `plan-validation` tests for id-bearing paths on tasks, contexts, edges and sources; lint warning format.
- 3.3: `criterion-records` schema accepts `covers`; `delivery-plan-binding-lint` (renamed coverage lint) tests for each code; propose derives claims and freezes them; the delivery gate attributes a verdict citing a record to the criteria it covers; prompt rendering shows `covers` beside the record.
- 3.4: finalizer emits per-context excerpts and scoped entries; `seededDocuments` accepted, capped and materialized; the locator lint skips seeded paths; `accessPolicy` absent from injected sources and the exemption deleted.
- 3.5: receipt tests for `spec start`, abandon transition on the coordinator, `nextAct` after abandon, `requires-pause` message, propose refusal on a seed-stub charter, outline hints.
...
- Docs: `scripts/graph-workflow-planner-docs.test.ts` gains assertions for the new planning-skill section and the `covers` field name paired with `criterionRecordSchema`, following its existing pattern of pairing documented terms with the schema that declares them.

## 7. Sequencing

Small, independently landable steps, ordered by leverage and by what the next step assumes:

1. 3.8 pause-before-provisioning fix (own ticket), so recovery recipes hold.
2. 3.1 replace merges server-owned fields, plus 3.5 seed charter and propose check. From here a spec delivery is authored as a `plan.json`.
3. 3.2 id-bearing locators and 3.5 outline hints, receipts, abandon transition, `requires-pause` reason.
4. 3.3 `covers` and derived claims, with the lint rename and the skill sections in 3.6.
5. 3.4 excerpts, seeded documents, locator lint, `accessPolicy` removal.
6. 3.7 authored-subset hash and the review line on propose.

Steps 2 and 3 alone remove most of the measured accidental cost. Step 4 removes the second authoring surface. Steps 5 and 6 are the token and review improvements.

## 8. Decisions for Alex

- **D-A** Derive claims from `covers` (3.3) and retire the authored claims sidecar, amending decision 2 of 2026-08-14. The alternative is to keep claims authored and add `covers` as advisory traceability only. Recommendation: derive; a second write surface is the cost this ticket exists to remove.
- **D-B** Hide the spec-side execution id from agent-facing output (3.5). Recommendation: hide; it mirrors the slug-only decision the memory system took for note ids.
- **D-C** Per-context spec excerpts derived from coverage (3.4) versus a planner-authored subset locator. Recommendation: derive; it costs the planner nothing and cannot drift from the claims.
- **D-D** Seeded documents for every plan (3.4). Recommendation: yes; it generalizes an SDD-only mechanism and closes a documented gap for ordinary plans.
- **D-E** Review hash over the authored subset (3.7). Recommendation: yes; it is the only way a review can follow a plan onto the managed definition.
