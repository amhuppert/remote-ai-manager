...
**Verdict attribution stays server-side.** Validators cite `criterionId` in blocking issues; the model-facing verdict schema does not gain positive citations or a `covers` field. GO certifies the whole context and proves the union of its frozen, selected coverage. A cited failure maps only to that record's covered spec criteria for failure presentation; a failed context grants no partial delivery. Claim satisfaction remains existential across stable contexts, and ordinary graph completion requirements still apply. This contract was clarified by Alex during the #109 design review on 2026-09-05.
...
Migration: binding schema version 4 carries `dispositions` only; the candidate manifest carries derived `claims`. Every unlaunched v3 attempt, including approved and parked attempts, must reopen into a v4 draft and re-propose; previously signed candidates require fresh human sign-off. A v3 launch refuses with the reopen remedy, preserving exact historical candidate bytes and approvals. Launched v3 attempts stay readable; the gate reads claims from the manifest for both versions. A `claims` key on any remaining binding write is refused with a message pointing at `covers`.
...
The claims map is one immutable document, `.cc/graph-workflow-docs/spec-bindings/<candidate>/claims.md`, referenced by a global source entry. It contains a context index and deterministic context sections; each context's prompt points directly to its section before referring to the full ownership map. Each spec criterion may have several stable context claimants, and the complete map remains available for deferral checks. There is no second per-context claims document; the per-context entry names the spec excerpt only. Alex approved this presentation during the #109 design review on 2026-09-05.
...
`canonicalPlanDefinitionHash` hashes the authored subset of managed definitions: remove server-owned `origin`, `approvalRequired`, `lockedRegions`, sources and seeded documents, using the ownership classification shared with finalization and the same `authoredDeliveryPlanSources` rank recovery. Ordinary definitions retain their full existing hashes, including any authored provenance, approval policy and locks. Authored seeded document contents remain in the review hash. Candidate and sign-off hashes still cover the exact finalized definition. A review recorded on the planner's `plan.json` then matches the managed definition; `spec plan propose` prints the advisory review line that `create` and `replace` already print, and the sign-off receipt and Spec Studio's delivery bridge show the verdict with the findings command. The acknowledgement gate keeps its current scope (create and replace) and is not extended to propose.
...
- Binding v3 to v4: dispositions only, claims derived into the candidate manifest. Every unlaunched attempt reopens and re-proposes; approved and parked candidates require fresh v4 sign-off. Historical candidate bytes and approvals remain intact, and launched attempts stay readable through the v3 parser. No table migration: the manifest is stored as JSON with its own `schemaVersion`.
...
- S4, step 4: a plan with `covers` proposes; the manifest carries derived claims; a satisfied context attributes evidence to exactly its frozen covered union; a cited failure maps only to the cited record's covered criteria and grants no partial delivery.
...
- 3.4: finalizer emits per-context excerpts and scoped entries; the claims document has a context index and deterministic sections, and prompts identify the reader's section; `seededDocuments` accepted, capped locally and at accept time, and materialized; the locator lint skips seeded paths; `accessPolicy` absent and the exemption deleted.
...
## 11. Ticket #109 implementation and verification — 2026-09-05

Steps 4 to 6 are implemented in the #109 session worktree and ready for review. The approved design-review decisions preserve whole-context GO, one indexed claims document, and fresh v4 sign-off for every unlaunched v3 candidate. Live scenarios S4 and S5 and the focused contract/regression checks are complete. The [verification report](../reports/2026-09-05_ticket109-verification.md) records the exact evidence, the failed-round replay boundary, and the broader suite's timeout limitation. Section 10 remains the historical record of the earlier #80 merge; this #109 work has not been merged.
