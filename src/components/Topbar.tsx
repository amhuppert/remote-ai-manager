...
const logger = createClientLogger("topbar");
...
export default function Topbar({
  ...
  const changeOpenSwitcher = (next: OpenSwitcher): void => {
    if (next !== null) captureOpener();
    setOpenSwitcher(next);
  };
  useAppHotkey("switchProject", () => {
    logger.info("topbar.switcher.opened", {
      source: "hotkey",
      switcher: "project",
    });
    changeOpenSwitcher("project");
  });
  useAppHotkey(
    "switchSession",
    () => {
  ...
  const changeNeedsYouOpen = (open: boolean): void => {
    if (open) captureOpener();
    setNeedsYouOpen(open);
  };
  const ticketsActive = pathname?.startsWith("/tickets") ?? false;
  const ticketsIndexActive = pathname === "/tickets";
  const specsActive = pathname?.startsWith("/specs") ?? false;
  const specsIndexActive = pathname === "/specs";
  ...
  const conversationsActive = pathname?.startsWith("/conversations") ?? false;

  const navigateFromHotkey = (commandId: string, href: string): void => {
    logger.info("topbar.navigation.requested", {
      commandId,
      destination: href,
      source: "hotkey",
    });
  ...
  useAppHotkey(
    "goProjects",
    () => navigateFromHotkey("goProjects", "/projects"),
    { enabled: !projectsActive },
  );
  useAppHotkey(
    "goConversations",
    () => navigateFromHotkey("goConversations", "/conversations"),
    { enabled: !conversationsActive },
  );
  useAppHotkey(
    "goTickets",
    () => navigateFromHotkey("goTickets", ticketsHref),
    { enabled: !ticketsIndexActive },
  );
  useAppHotkey("goSpecs", () => navigateFromHotkey("goSpecs", specsHref), {
    enabled: !specsIndexActive,
  });
  useAppHotkey(
    "openNeedsYou",
    () => {
      logger.info("topbar.needs_you.opened", {
        itemCount: needsYouItems.length,
        source: "hotkey",
      });
      changeNeedsYouOpen(true);
    },
    {
      enabled: hydrated && needsYouItems.length > 0,
    },
  );
    ...
    <>
                ...
                >
                  ...
                  {seg.isProject ? (
                    <BreadcrumbProjectSwitcher
                      ...
                      onOpenChange={(open) =>
                        changeOpenSwitcher(open ? "project" : null)
                      }
                    ...
                    <BreadcrumbSessionSwitcher
                      ...
                      onOpenChange={(open) =>
                        changeOpenSwitcher(open ? "session" : null)
                      }
        ...
        <div className="flex items-center gap-md max-[360px]:gap-xs max-768:gap-sm">
          {hydrated && (
            <NeedsYouMenu
              ...
              nowMs={needsYouNowMs}
              open={needsYouOpen}
              onOpenChange={changeNeedsYouOpen}
              restoreOpener={restoreOpener}
            />
          )}
      ...
      {projectSegment === undefined ? (
        <GlobalProjectSwitcher
          activeProject={null}
          open={openSwitcher === "project"}
          onOpenChange={(open) => changeOpenSwitcher(open ? "project" : null)}
          destination={page === "specs" ? "specs" : "project"}
          captureOpener={captureOpener}
          restoreOpener={restoreOpener}
        />
