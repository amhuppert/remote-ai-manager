...
/** POST /api/projects/[name]/sessions/[session]/conversations/[conversationId]/prompt */
export const POST = withTracing(async (request, { params }) => {
  ...
  const conversation = await getConversation(projectPath, sessionName, conversationId);
  if (!conversation) {
    return NextResponse.json(
