...
export default function WorkflowCard({
  ...
  return (
    <Link
      ...
      className="relative flex flex-col gap-sm overflow-hidden rounded-lg border border-solid border-border-subtle bg-bg-surface p-lg text-inherit no-underline transition-all duration-150 ease-[ease] before:absolute before:top-0 before:right-0 before:left-0 before:h-px before:bg-[linear-gradient(90deg,transparent,var(--cyan-glow-strong),transparent)] before:opacity-0 before:transition-opacity before:duration-200 before:ease-[ease] before:content-[''] hover:-translate-y-[2px] hover:border-border-strong hover:bg-bg-raised hover:before:opacity-100"
    >
      <div className="flex items-center justify-between font-mono text-[0.7rem] text-text-tertiary">
        ...
        <span className="tracking-[0.08em] text-violet uppercase">
          {spec.character}
        </span>
      </div>
      ...
      <div className="mt-sm grid grid-cols-5 gap-sm border-x-0 border-t border-b-0 border-solid border-border-subtle pt-sm">
...
function Stat({
  ...
  return (
    <div className="flex flex-col items-start gap-[2px]">
      <span className="font-mono text-[1.1rem] text-cyan tabular-nums">
        {value}
      </span>
