...
// the agents' outputs own the whole viewport. Portaled to the document body so
// it escapes the transformed docked stage (docked-stage-transform-breaks-fixed)
// and layers above the rest of the app.
export default function CollabReaderOverlay({
  ...
  onClose,
  children,
}: CollabReaderOverlayProps): React.JSX.Element | null {
  ...
  return createPortal(
    <div
      ref={panelRef}
      className="fixed inset-0 z-overlay flex animate-[fadeIn_0.15s_ease] flex-col bg-bg-void outline-none"
      role="dialog"
      aria-modal="true"
