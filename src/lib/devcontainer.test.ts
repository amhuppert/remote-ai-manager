...
describe("resolveConfig", () => {
  ...
  it("returns CSM default config when project has no devcontainer.json", () => {
    ...
    expect(result.configPath).toContain("devcontainer-defaults/devcontainer.json");
  });
});
...
describe("buildContainerEnv", () => {
  ...
  const envKeys = ["ANTHROPIC_API_KEY", "CLAUDE_CODE_OAUTH_TOKEN", "CLAUDE_CODE_CONFIG"];
...
describe("prepareSessionEnvironment", () => {
  it("creates claude dir and writes hook script and settings", async () => {
    ...
    expect(mkdirMock).toHaveBeenCalledWith(expect.stringContaining("containers/"), {
      recursive: true,
    });

    // Should write hook script
    const hookCall = writeFileMock.mock.calls.find(
      (c: unknown[]) => String(c[0]).includes("csm-hook.sh"),
    );
    ...
    const settingsCall = writeFileMock.mock.calls.find(
    ...
    const onboardingCall = writeFileMock.mock.calls.find(
      (c: unknown[]) => String(c[0]).includes("onboarding.json"),
    );
  ...
  it("does not copy credentials (bind-mounted at container start instead)", async () => {
    ...
    const writeTargets = writeFileMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(writeTargets.every((t: string) => !t.includes(".credentials.json"))).toBe(true);
  });
});
