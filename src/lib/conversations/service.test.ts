  ...
  return async (_ref, input) => {
    ...
    if (!seed) {
      throw new ContinuityForkError(
        backend,
        "Fork creation failed: the synthetic seed could not be built",
      );
    }
...
function makeFakeContinuity(backend: "claude" | "codex", fork: FakeForkImpl) {
  ...
  const adapter: BackendContinuityAdapter = {
    ...
    fork: async (ref, input) => {
      forkCalls.push({ ref, input });
      return fork(ref, input);
    },
  };
  return { adapter, forkCalls };
}
...
function createTestServices(
  ...
  const claudeContinuity = makeFakeContinuity(
    "claude",
    overrides.claudeFork ??
      (async () => ({
        kind: "native",
        ref: { backend: "claude", ref: "fake-forked-session-id" },
      })),
  );
  const codexContinuity = makeFakeContinuity(
    "codex",
...
describe("forkConversation", () => {
    ...
    await seedSession(state, {
      conversations: [
        makeConvo({
          ...
          status: "awaiting",
          promptCount: 2,
          backendRef: { backend: "claude", ref: "src-session-abc" },
          ...sourceOverrides,
        }),
      ],
    });
  ...
  it("case 1: fork at assistant message — inclusive copy, anchor on assistant UUID, eager native fork", async () => {
    ...
    const { state, conversations, claudeContinuity } = createTestServices({
      claudeFork: async () => ({
        kind: "native",
        ref: { backend: "claude", ref: "forked-1" },
      }),
    });
    ...
    expect(claudeContinuity.forkCalls[0]).toEqual({
      ref: { backend: "claude", ref: "src-session-abc" },
      input: {
        projectPath: "/proj",
  ...
  it("case 2: fork at user message N>0 — exclusive copy, anchor on prior assistant UUID, eager native fork, pendingPromptText set", async () => {
    ...
    const { state, conversations, claudeContinuity } = createTestServices({
      claudeFork: async () => ({
        kind: "native",
        ref: { backend: "claude", ref: "forked-2" },
      }),
    });
  ...
  it("case 3: fork at user message index 0 — empty transcript, no source backend ref, no continuity fork call, pendingPromptText set", async () => {
    const sourceId = crypto.randomUUID();
    const transcriptPath = path.join(TEST_DIR, "source.jsonl");
    const { state, conversations, claudeContinuity } = createTestServices();
  ...
  it("case 4 (assistant fork): synthetic_seed outcome — backendRef null, forkMode synthetic, pendingPromptText holds the seed", async () => {
    ...
    const transcriptPath = path.join(TEST_DIR, "source.jsonl");
    const { state, conversations, claudeContinuity } = createTestServices({
      claudeFork: syntheticForkImpl("claude"),
    });
  ...
  it("case 4: typed ForkCreationError when the continuity adapter produces no outcome — no conversation is created", async () => {
    const { ForkCreationError } = await import("./service");
    const sourceId = crypto.randomUUID();
    const transcriptPath = path.join(TEST_DIR, "source.jsonl");

    // Initial validation read at the top of forkConversation() succeeds.
    // The injected fork deletes the transcript file before running the real
    // synthetic path, so buildSyntheticForkSeed returns null and the adapter
    // throws ContinuityForkError — exercising the typed-error path.
    const claudeFork: FakeForkImpl = async (ref, input) => {
      await rm(transcriptPath, { force: true });
      return syntheticForkImpl("claude")(ref, input);
    };
    const { state, conversations, claudeContinuity } = createTestServices({
      claudeFork,
    });
  ...
  it("legacy transcript without assistant UUID: passes a null anchor to the adapter and surfaces the synthetic outcome", async () => {
    // When the transcript predates UUID storage (or the assistant entry lacks
    // a uuid), findForkAnchorUuid returns null. The service must hand the
    // adapter a null anchorMessageId — never a fabricated one — because an
    // unanchored native fork would silently carry the full source history
    // past the visible fork point.
    const sourceId = crypto.randomUUID();
    const transcriptPath = path.join(TEST_DIR, "source.jsonl");
    const { state, conversations, claudeContinuity } = createTestServices({
      claudeFork: syntheticForkImpl("claude"),
    ...
    expect(fork).toBeTruthy();
    expect(claudeContinuity.forkCalls).toHaveLength(1);
    expect(claudeContinuity.forkCalls[0]!.input.anchorMessageId).toBeNull();
    expect(fork!.backendRef).toBeNull();
    expect(fork!.forkedFrom!.forkMode).toBe("synthetic");
  ...
  it("codex source: fork produces a synthetic seed (forkMode synthetic, backendRef null, seed in pendingPromptText)", async () => {
    ...
    await seedWithSourceTranscript(
      ...
      transcriptPath,
      [
        userEntry("codex question", "2024-01-01T00:00:00Z"),
        assistantEntry("codex answer", "uuid-a1", "2024-01-01T00:00:01Z"),
      ],
      {
        agentBackend: "codex",
        backendRef: { backend: "codex", ref: "thread-src-1" },
      },
    );
