/**
 * Route handler for submitting answers to Claude's pending AskUserQuestion.
 *
 * Resolves the deferred answer promise held in the runtime state so the
 * SDK's `canUseTool` callback can return, then sends an ANSWER event to
 * the conversation machine for state tracking.
 */
