 ...
 *
 * A schema-bearing phase runs the facade's two-turn protocol: prose work,
 * then a format turn carrying the per-phase `outputFormat` derived from
 * `schemas.ts` (see the conversation lifecycle module).
 */
...
2. **Wait for Reproduction**: Give clear ordered reproduction steps; the follow-up format turn records them as the \`reproductionSteps\` array, and the UI renders the steps deterministically from that field. Do not continue until the user says reproduction is complete.
...
4. Do NOT delete \`{DEBUG_MANIFEST_PATH}\` yourself; Command Center cross-checks your structured report against the manifest and removes the file only after the verification passes. Confirm in your response that you understand CC owns the deletion; the follow-up format turn records it as \`acknowledgesManifestDeletionContract: true\`.
