...
export interface SessionEnvContractInput {
  ...
  /** Actual ordinary caller, distinct from a collaboration runtime's routing target. */
...
export function buildSessionEnvContract(
  ...
  if (input.workflowCallerConversationId !== undefined) {
    if (input.workflowExecutionId !== undefined) {
      throw new Error(
        "buildSessionEnvContract: a workflow lane must not also carry an ordinary conversation identity",
      );
    }
    if (input.target.scope !== "session") {
      throw new Error(
        "buildSessionEnvContract: a project conversation has no session to launch into and must not carry a workflow caller identity",
      );
    }
    env[CONVERSATION_IDENTITY_ENV_VAR] = input.workflowCallerConversationId;
  }
