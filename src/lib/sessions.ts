...
async function updateContainerStatus(
  ...
  fields?: Partial<Pick<SessionState, "containerId" | "containerError" | "claudeHostDir">>,
): Promise<void> {
  session.containerStatus = status;
  if (fields?.containerId !== undefined) session.containerId = fields.containerId;
  if (fields?.containerError !== undefined) session.containerError = fields.containerError;
  if (fields?.claudeHostDir !== undefined) session.claudeHostDir = fields.claudeHostDir;
