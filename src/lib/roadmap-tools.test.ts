...
const mockCreateRoadmapItem = vi.fn();
const mockDeleteRoadmapItem = vi.fn();
const mockGetRoadmapItems = vi.fn();

vi.mock("@/lib/state", () => ({
  createRoadmapItem: (...args: unknown[]) => mockCreateRoadmapItem(...args),
  deleteRoadmapItem: (...args: unknown[]) => mockDeleteRoadmapItem(...args),
  getRoadmapItems: (...args: unknown[]) => mockGetRoadmapItems(...args),
}));
...
describe("roadmap-tools", () => {
  ...
  describe("add_roadmap_item", () => {
    it("creates an item and returns a success summary", async () => {
      ...
      mockCreateRoadmapItem.mockResolvedValue(createdItem);

      createRoadmapToolServer({ projectPath: "/projects/test" });
      ...
      expect(mockCreateRoadmapItem).toHaveBeenCalledWith("/projects/test", {
        title: "Add dark mode",
        type: "feature",
    ...
    it("passes description when provided", async () => {
      ...
      mockCreateRoadmapItem.mockResolvedValue(
    ...
    it("returns isError when state mutation fails", async () => {
      ...
      mockCreateRoadmapItem.mockRejectedValue(new Error("State write failed"));
  ...
  describe("remove_roadmap_item", () => {
    ...
    it("returns isError when item ID is not found", async () => {
      ...
      mockDeleteRoadmapItem.mockRejectedValue(
  ...
  describe("list_roadmap_items", () => {
    it("returns formatted list of non-archived items grouped by type", async () => {
      ...
      mockGetRoadmapItems.mockResolvedValue([
