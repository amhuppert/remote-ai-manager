...
function getSessionClaudeDir(sessionName: string): string {
  const hash = crypto
    ...
    .digest("hex")
    .slice(0, 12);
  return path.join(getConfigDirPath(), "containers", `${sanitizeForDocker(sessionName)}-${hash}`);
}
...
function parseDevcontainerUpOutput(stdout: string): ContainerInfo {
    ...
    if (line.startsWith("{")) {
      try {
        ...
        if (parsed.outcome === "error") {
          throw new Error(
            `devcontainer up failed: ${JSON.stringify(parsed)}`,
          );
        }
  ...
  throw new Error(`Failed to parse devcontainer up output: ${stdout.slice(0, 500)}`);
}
