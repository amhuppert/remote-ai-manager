...
let mockState: ManagerState = { projects: {}, archivedProjects: [], pinnedProjects: [] };
...
describe("processHookEvent", () => {
  ...
  it("links hook to running CSM conversation instead of creating duplicate", async () => {
    ...
    expect(convo.transcriptPath).toBe("/home/user/.claude/transcripts/456.jsonl");
  });
  ...
  it("creates new conversation when no running CSM conversation exists", async () => {
    ...
    const newConvo = session.conversations.find(c => c.claudeSessionId === "cli-session-789");
