 ...
 * that references the affected `serverKey` may need to re-render so orphan
 * flags, tool filter states, and the live tool list surface correctly. The
 * cache itself is inert: it exposes an `onCompletion` hook but never decides
 * what "scoped view" means. This module is the concrete caller that bridges
 * the two.
 *
...
export interface ScopedMcpViewInvalidator {
  invalidate(input: { serverKey: string }): void;
}
