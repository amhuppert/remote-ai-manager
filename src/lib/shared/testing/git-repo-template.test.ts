...
import { createGitRepoTemplate, type GitRepoTemplate } from "./git-repo-template";
...
describe("createGitRepoTemplate", () => {
  ...
  it("hands out independent copies of one built repository, symlinks included", async () => {
    template = await createGitRepoTemplate("cc-template-test-", async (repo) => {
    ...
    expect((await lstat(path.join(second, "link.txt"))).isSymbolicLink()).toBe(true);
    expect(await readFile(path.join(second, "link.txt"), "utf8")).toBe("alpha\n");
