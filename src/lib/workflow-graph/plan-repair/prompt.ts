...
export function buildPlanRepairPrompt(input: PlanRepairPromptInput): string {
  ...
  sections.push(
    [
      ...
      "",
      "Emit live-edit operations from this vocabulary ONLY (plan artifacts; no structural graph changes, no validator/gate/config controls):",
      '- `amend-charter` — partial charter content merge; `rationale` is REQUIRED and is recorded in the amendment log every future agent sees.',
      '- `update-context` — `title`, `description`, `acceptanceCriteria`, `iterationPolicy`, `circuitBreaker` only.',
      "- `add-task`, `update-task`, `remove-task`, `reorder-tasks` — task-level plan repairs in any non-completed context.",
      "",
      "## Output",
      "",
