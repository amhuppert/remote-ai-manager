...
export function buildPlanRepairPrompt(input: PlanRepairPromptInput): string {
  ...
  sections.push(
    [
      ...
      '{"type": "update-context", "contextId": "<id>", "title": "...", "description": "...", "acceptanceCriteria": [{"id": "<kebab-case>", "statement": "..."}], "outputSchema": {"type": "object", "properties": {}} /* or null to drop it */, "placement": {"lane": "<laneId>", "mode": "full" | "readOnly" | "owned", "ownedPaths": ["<repo-relative path>"] /* mode "owned" only */} /* replaces the placement wholesale */, "iterationPolicy": {"maxIterations": 10}, "circuitBreaker": {"consecutiveFailureThreshold": 3}}  // include only the fields you are changing',
