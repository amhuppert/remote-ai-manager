...
describe("execution loop", () => {
  ...
  it.each([
    ...
    async ({ grade, expectResync }) => {
      // An owned landing writes back only the entries it committed (D7), so
      // any other drift is still there — and for a path the index has no entry
      // for, `git commit -a`, which a full-access agent may run, would publish
      // that file's deletion. Repairing the index is therefore part of handing
      // the lane to a whole-tree writer, and only to one: an enveloped member
  ...
  it.each([
    ...
    },
    {
      ...
      unattributedPaths: ["scripts/deploy.sh", "src/server/router.ts"],
      expectHalt: true,
    },
  ])(
    ...
    async ({ unattributedPaths, expectHalt }) => {
      ...
      // The audit judges the lane against the frozen union, relativized.
