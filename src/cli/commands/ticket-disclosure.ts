...
export function relationshipGetCommand(
  ...
  return `cctl ticket relation get ${quoteAgentCommandArgument(identifier)} ${quoteAgentCommandArgument(relationshipId)}`;
}
...
export function statusUpdateGetCommand(
  ...
  return `cctl ticket status-update get ${quoteAgentCommandArgument(identifier)} ${quoteAgentCommandArgument(updateId)}`;
}
...
export function relationshipListCommand(
  ...
  const parts = [
    "cctl ticket relation list",
    quoteAgentCommandArgument(identifier),
  ];
  ...
  if (options.cursor !== undefined) {
    parts.push("--cursor", quoteAgentCommandArgument(options.cursor));
  }
  return parts.join(" ");
}
...
export function statusUpdateListCommand(
  ...
  const parts = [
    "cctl ticket status-update list",
    quoteAgentCommandArgument(identifier),
    "--limit",
...
export function buildRelationshipOutline(
  ...
  return ticketRelationshipOutlineSchema.parse({
    ...
    getCommand: relationshipGetCommand(identifier, relationship.id),
  });
}
...
export function buildStatusUpdateOutline(
  ...
  return ticketStatusUpdateOutlineSchema.parse({
    ...
    getCommand: statusUpdateGetCommand(identifier, update.id),
  });
}
...
export function buildRelationshipPageProjection(
  ...
  const items = parsed.items.map((item) =>
    buildRelationshipOutline(item, identifier),
  );
  const metadata =
...
export function buildStatusUpdatePageProjection(
  ...
  const items = parsed.items.map((item) =>
    buildStatusUpdateOutline(item, identifier),
  );
  const metadata =
    parsed.nextCursor === null
      ? completeMetadata(parsed.total, items.length)
...
export function buildTicketGetProjection(
  ...
  const relationshipItems = [...parsed.relationships]
    ...
    .map((item) => buildRelationshipOutline(item, identifier));
...
export async function emitTicketDisclosure(input: {
  ...
  const envelope = { ok: true as const, ...input.payload };
  const selected = input.json ? `${JSON.stringify(envelope)}\n` : input.text;
  const outcome = await emitLarge(input.host, selected, {
    format: input.json ? "json" : "text",
    namePrefix: input.namePrefix,
    budgetBytes: STDOUT_BUDGET_BYTES,
  });
  ...
  if (outcome.kind === "unwritable") {
    return outcome.reason === "host_cannot_write"
      ? failure({
          ...
          message: `${input.command}: this CLI host cannot write artifact files`,
          code: "write_unavailable",
          json: input.json,
        })
      : failure({
          exitCode: EXIT_OPERATION_FAILED,
          message: `${input.command}: could not write ${JSON.stringify(outcome.path)}`,
          code: "write_failed",
          json: input.json,
        });
  }
  const receipt = ticketArtifactReceiptSchema.parse({
    ok: true,
  ...
  return {
    ...
    stdout: input.json
      ...
      : artifactReceiptText(input.command, outcome.manifest),
    stderr: "",
  };
}
