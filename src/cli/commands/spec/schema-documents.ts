...
const IMPORT_DRY_RUN_USAGE = "cctl spec import-preview --file <bundle.json>";
...
function importBundleDocument(): SchemaDocument {
  ...
  return {
    ...
    notes: [
      ...
      "Handles are allocated in bundle order: requirements R1…Rn, criteria numbered within each requirement (R1.1, R1.2), decisions D1…Dn, questions Q1…Qn, assumptions A1…An. Run spec import-preview to see the exact numbering before writing cross-references against it.",
      "dryRun: true rehearses the whole import — every validation, the findings, and the handles it would allocate — and writes nothing. Use spec import-preview for rehearsal; spec import requires dryRun false and spec import-check validates admission without committing.",
    ],
  };
}
...
export function authoringSchemaDocuments(): SchemaDocument[] {
  return [
    ...elementDocuments(),
    batchDocument(),
