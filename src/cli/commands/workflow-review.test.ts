...
const PLAN = { name: "X", definition: {}, layout: {} };
...
describe("cctl workflow review (status mode)", () => {
  it("posts the whole plan to the status route and prints the verdict, findings, and reader commands", async () => {
    ...
    const result = await runCli(
      ["workflow", "review", "--file", PLAN_FILE],
      baseEnv,
      host,
    );
    ...
    expect(result.stdout).toContain(
      `plan review: changes_requested by ${REVIEWER}`,
    );
  ...
  it("reports an unreviewed revision plainly and exits 0", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("none recorded for this revision");
    expect(result.stdout).toContain(HASH);
  ...
  it("carries the whole status, reader commands included, in the --json envelope", async () => {
    const host = makeHost(() => jsonResponse(reviewedStatus()));
    const result = await runCli(
      ["workflow", "review", "--file", PLAN_FILE, "--json"],
      baseEnv,
      host,
    );
  ...
  it("exits 2 when --file is missing, before any request", async () => {
    const host = makeHost(() => jsonResponse({}));
    const result = await runCli(["workflow", "review"], baseEnv, host);
...
describe("cctl workflow review (record mode)", () => {
  ...
  it("records a changes-requested verdict with the findings file text, mapping the flag onto the stored enum", async () => {
    ...
    expect(result.stdout).toContain(`cctl workflow review --file ${PLAN_FILE}`);
  });
  ...
  it("records an approved verdict with no findings", async () => {
    ...
    const result = await runCli(
      ["workflow", "review", "--file", PLAN_FILE, "--verdict", "approved"],
      baseEnv,
      host,
    );
  ...
  it("exits 2 on an unknown verdict without spending a request", async () => {
    const host = makeHost(() => jsonResponse({}));
    const result = await runCli(
      ["workflow", "review", "--file", PLAN_FILE, "--verdict", "rejected"],
      baseEnv,
      host,
    );
    expect(result.exitCode).toBe(2);
  ...
  it("exits 2 when the findings file cannot be read", async () => {
    const host = makeHost(() => jsonResponse(receipt, 201));
    const result = await runCli(
      [
        "workflow",
    ...
    expect(result.exitCode).toBe(2);
    expect(result.stderr).toContain("cannot read findings file");
    expect(host.requests).toHaveLength(0);
  });
});
...
describe("review advisory on create and replace", () => {
  ...
  it("prints one advisory line beside a create and still exits 0", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain(
      `plan review: approved by ${REVIEWER} at 2026-08-18T12:00:00.000Z`,
    );
  ...
  it("replaces against a changes-requested verdict without refusing", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("replaced Auth Setup");
    expect(result.stdout).toContain(
      `plan review: changes_requested by ${REVIEWER}`,
    );
  });
...
describe("acknowledgement gate on create and replace", () => {
  ...
  it("sends no acknowledgement field when the flag is absent", async () => {
    const host = makeHost(() => jsonResponse({ item }, 201));
    await runCli(["workflow", "create", "--file", PLAN_FILE], baseEnv, host);
  ...
  it("renders the create refusal with the expected hash and a runnable findings hint", async () => {
    ...
    expect(result.stderr).toContain(`reviewer: ${REVIEWER}`);
    // The findings command carries THIS caller's plan path, not the server's
    // placeholder — the whole point is that it can be pasted as-is.
    expect(result.stderr).toContain(
      `hint: read the findings with 'cctl workflow review --file ${PLAN_FILE}', then revise the plan or re-run with --acknowledge-review ${HASH}`,
    );
  ...
  it("renders the replace refusal the same way", async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain(HASH);
    expect(result.stderr).toContain(`cctl workflow review --file ${PLAN_FILE}`);
    expect(result.stderr).toContain(`--acknowledge-review ${HASH}`);
  });
  ...
  it("carries the code, details, and hint in the --json envelope", async () => {
    ...
    expect(envelope.code).toBe("review-changes-requested-unacknowledged");
    expect(envelope.details).toMatchObject({
      definitionHash: HASH,
      verdict: "changes_requested",
      reviewerConversationId: REVIEWER,
    });
    expect(envelope.hint).toContain(`cctl workflow review --file ${PLAN_FILE}`);
  });

  it("falls back to the plain server refusal when the payload is not the gate shape", async () => {
    const host = makeHost(() =>
