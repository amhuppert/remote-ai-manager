import { workflowCommands } from "./definitions";
...
describe("server outline contract through the CLI", () => {
  it.each([
    ...
    async ({ selector, args, marker }) => {
      ...
      const jsonResult = await runCcWithHost(
        ["workflow", "live", "get", ...args, "--json"],
        baseEnv,
        makeHost(() => jsonResponse(body)),
      );
      expect(jsonResult.exitCode).toBe(0);
      expect(JSON.parse(jsonResult.stdout).payload.data).toMatchObject(body);
    },
  );
});
...
describe("cctl workflow live command declarations", () => {
  it.each(["execution", "exec"])(
    "does not declare the retired %s surface",
    (alias) => {
      expect(
        workflowCommands.map((command) => command.spec.path),
      ).not.toContain(`workflow ${alias} get`);
    },
  );
});
...
describe("cctl workflow live get", () => {
  ...
  it("renders the text outline (header, contexts, tasks, config)", async () => {
    ...
    expect(result.stdout).toMatchInlineSnapshot(`
      ...
      config:
        plan    claude opus effort=medium; validator general claude sonnet effort=medium; script off
        impl    claude opus effort=medium; validator general claude sonnet effort=medium; script typecheck+test; roles implementer all-except format, validator none; approval on
      ...
      staffing (snapshots):
        ...
        plan    validator    general           builtin:general-reviewer@5     #eeeeeeeeeeee  claude sonnet effort=medium
        impl    implementer  implementer       builtin:general-implementer@2  #cccccccccccc  claude opus effort=medium
        impl    validator    general           builtin:general-reviewer@5     #eeeeeeeeeeee  claude sonnet effort=medium
        ...
        verify  validator    dormant-security  global:house-reviewer@3        #ffffffffffff  codex gpt-5.4 fast=false reasoning=low  (cohort disabled)
  ...
  it("renders one staffing row per seeded assignment with revision and hash", async () => {
    ...
    expect(staffing?.[1]).toContain("claude sonnet effort=medium");
   ...
   * prohibits. The response is rejected when the renderer cannot establish
  ...
  it("renders an explicit lane-merge command list, and 'none' when empty", async () => {
    ...
    const only = await runCcWithHost(
      ["workflow", "live", "get"],
      baseEnv,
      hostOnly,
    );
    ...
    const none = await runCcWithHost(
      ["workflow", "live", "get"],
      baseEnv,
      hostNone,
    );
  ...
  it("passes the endpoint JSON through the envelope with --json", async () => {
    ...
    expect(parsed.payload.data.section).toBe("outline");
    expect(parsed.payload.data.outline.header.executionId).toBe("exec-7");
  });
...
describe("cctl workflow live get --outputs (R7.2)", () => {
  ...
  it("passes the outputs payload through the --json envelope", async () => {
    ...
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
    expect(parsed.payload.data.section).toBe("outputs");
    expect(parsed.payload.data.outputs[0].capture.value).toEqual({
      verdict: "pass",
      notes: "all green",
    });
  });

  it("refuses malformed output evidence without guessing its shape", async () => {
    const host = makeHost(() =>
...
describe("cctl workflow live edit", () => {
  ...
  it("posts the parsed body with source cli and renders the applied count", async () => {
    ...
    expect(result.stdout).toContain("Applied: 1 operations");
    expect(result.stdout).toContain("liveRevision 5");
  });
  ...
  it("sets dryRun on the body with --dry-run", async () => {
    ...
    const result = await runCcWithHost(
      ["workflow", "live", "edit-preview", "--file", opsFile],
      baseEnv,
      host,
    );
    expect(result.exitCode).toBe(0);
  ...
  it("maps a code-bearing rejection to exit 1 with the code and issues", async () => {
    ...
    expect(JSON.parse(jsonResult.stdout).error.details.serverCode).toBe(
      "revision_conflict",
    );
  });
...
describe("cctl workflow live pause / resume", () => {
  it("posts to the pause endpoint", async () => {
    const host = makeHost(() =>
      jsonResponse({ execution: { executionId: "exec-7", status: "paused" } }),
    );
    const result = await runCcWithHost(
      ["workflow", "live", "pause"],
      baseEnv,
  ...
  it("posts to the resume endpoint", async () => {
    const host = makeHost(() =>
      jsonResponse({ execution: { executionId: "exec-7", status: "running" } }),
    );
    const result = await runCcWithHost(
...
describe("cctl workflow live abort", () => {
  it("posts the reason to the abort endpoint and claims no separate release step", async () => {
    const host = makeHost(() =>
      jsonResponse({ execution: { executionId: "exec-7", status: "aborted" } }),
    );
    const result = await runCcWithHost(
  ...
  it("reports the abort as having released the slot", async () => {
    const host = makeHost(() =>
      jsonResponse({
        execution: { executionId: "exec-7", status: "aborted" },
        released: true,
      }),
    );
    const result = await runCcWithHost(
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      payload: { data: { execution: { status: "aborted" }, released: true } },
    });
  });
});
...
describe("cctl workflow abandon", () => {
  ...
  it("posts the execution id and reason with signed mutation identity", async () => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      effect: "applied",
      payload: { data: { abandoned: true, execution } },
    });
  });

  it("does not declare a separate workflow live release mutation", () => {
    expect(workflowCommands.map((command) => command.spec.path)).not.toContain(
      "workflow live release",
    );
  });
});
...
describe("cctl workflow live ledger (D4 R16.2)", () => {
  ...
  it("returns the derived ledger as JSON", async () => {
    ...
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
    expect(parsed.payload.data.ledger).toHaveLength(1);
    expect(parsed.payload.data.ledger[0].loopGroupId).toBe("refine");
    expect(
      parsed.payload.data.ledger[0].decisions.map(
  ...
  it("walks past any fixed page cap until the reader reports exhaustion", async () => {
    ...
    const parsed = JSON.parse(result.stdout);
    expect(parsed.payload.data.walk.complete).toBe(true);
    expect(parsed.payload.data.walk.resumeCursor).toBeNull();
    ...
    const passTwo = parsed.payload.data.ledger[0].decisions.filter(
  ...
  it("bounds a walk on request and names the cursor to resume from", async () => {
    ...
    const parsed = JSON.parse(result.stdout);
    expect(parsed.payload.data.walk.complete).toBe(false);
    expect(parsed.payload.data.walk.resumeCursor).toBe(3);
  });
  ...
  it("points at the resume read through the shared disclosure vocabulary", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("--cursor=2");
  ...
  it("discloses a stalled walk without a resume pointer it cannot honour", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("history incomplete (reader-stalled)");
    expect(result.stdout).not.toContain("note:");
  ...
  it("discloses a bounded walk even when no loop group has an entry", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("--cursor=9");
  });
  ...
  it("stops instead of spinning when the reader repeats a cursor", async () => {
    ...
    expect(JSON.parse(result.stdout).payload.data.walk.complete).toBe(false);
  });
  ...
  it("says so plainly when the execution declares no loops", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("No loop decisions recorded.");
...
describe("live write receipts name baseLiveRevision (#80 I-7, I-12)", () => {
  ...
  it("names the token on the live outline the next edit is addressed from", async () => {
    ...
    expect(JSON.parse(structured.stdout).payload.data).toMatchObject({
      baseLiveRevision: 4,
    });
  });
