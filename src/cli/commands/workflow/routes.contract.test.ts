// @vitest-inputs src/cli/commands/spec/native-write-definitions.ts
// @vitest-inputs src/cli/commands/workflow/definitions.ts
...
function requiredFixture<T>(value: T | undefined): T {
  if (value === undefined)
    throw new Error("Required workflow fixture record is missing.");
  return value;
}
...
describe("cctl workflow against the real workflow route handlers", () => {
  it("list parses the definition route's { items } shape", async () => {
    ...
    expect(envelope.payload.data.items[0].id).toBe("wf-1");
    expect(envelope.payload.data.items[0].name).toBe("Auth Setup");
  });
  ...
  it("delete maps to the real DELETE route", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("Deleted workflow wf-1");
  });
  ...
  it("status parses the full execution route incl. a structured haltReason", async () => {
    ...
    expect(result.stdout).toContain("Halt: max_iterations");
  });
  ...
  it("status --json bounds the real route payload to the table's own projection", async () => {
    ...
    const envelope = JSON.parse(bounded.stdout).payload.data;
    ...
    expect(JSON.parse(full.stdout).payload.data.execution).toEqual(
  ...
  it("reads the same durable projection by explicit id from Current or History without capability authority", async () => {
    ...
    for (const location of ["Current", "History"]) {
      ...
      expect(
        JSON.parse(result.stdout).payload.data.execution,
        location,
      ).toEqual(JSON.parse(JSON.stringify(execution)));
    }
  });
    ...
    for (const json of [false, true]) {
      ...
      if (json) {
        expect(JSON.parse(result.stdout)).toMatchObject({
          payload: { data: { receipt: { warnings } } },
        });
      } else {
        expect(result.stdout).toContain(warnings[0]?.path);
        expect(result.stdout).toContain(warnings[0]?.message);
        expect(result.stdout).toContain("execution-warned-start");
      }
    }
  });
  ...
  it("forwards a lane capability so the real start route refuses nesting", async () => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      error: { details: { serverCode: "workflow_nesting_refused" } },
    });
  });
  ...
  it("runs a one-off plan through the real route and preserves the launch receipt without a definition id", async () => {
    ...
    expect(result.exitCode).toBe(0);
    const envelope = JSON.parse(result.stdout);
    expect(envelope.payload.data.receipt).toMatchObject({
      executionId: "execution-one-off",
      status: "running",
      origin: { kind: "one_off", planName: "One-off audit" },
      originConversationId: CLI_CONVERSATION_ID,
    });
    expect(envelope.payload.data.receipt.deepLink).toContain(
      "execution=execution-one-off",
    );
  ...
  it("renders real run-route warnings before text receipt and in JSON without changing exit 0", async () => {
    ...
    for (const json of [false, true]) {
      ...
      if (json) {
        ...
        expect(result.stdout).toContain(warnings[0]?.path);
        expect(result.stdout).toContain(warnings[0]?.message);
        expect(result.stdout).toContain("execution-warned-run");
      }
    }
  });
  ...
  it("forwards a lane capability so the real run route returns the shared blocker-free nesting refusal", async () => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      error: { details: { serverCode: "workflow_nesting_refused" } },
      instruction: expect.stringContaining("conversation"),
    });
  });
  ...
  it("reattaches by durable cursor through a reconstructed route host after the first wait times out", async () => {
    ...
    const timedOut = await runCcWithHost(
      [
        "workflow",
        "wait",
        execution.id,
    ...
    const reattached = await runCcWithHost(
      [
        "workflow",
        "wait",
        execution.id,
    ...
    expect(JSON.parse(reattached.stdout)).toMatchObject({
      effect: "read",
      payload: { data: { result: boundary, cursor: "42" } },
    });
  });
  ...
  it("forwards a lane capability so the current lane can pause its own run", async () => {
    ...
    const execution: GraphWorkflowExecution = {
      ...
      taskStates: {
        ...base.taskStates,
        "task-plan-1": {
          ...requiredFixture(base.taskStates["task-plan-1"]),
          status: "running",
  ...
  it("start treats the real approval-required route response as a successfully parked execution", async () => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      payload: {
        data: {
          execution: { executionId: "execution-review" },
  ...
  it("get prints the compact outline by default (sizes, not bodies)", async () => {
    ...
    expect(result.stdout).toContain(
      "Workflow Graph Builder (wf-1); revision 3",
    );
  ...
  it("get --task slices one task's full instructions", async () => {
    ...
    expect(result.exitCode).toBe(0);
    const envelope = JSON.parse(result.stdout);
    expect(envelope.payload.data.section).toBe("task");
    expect(envelope.payload.data.value.instructions).toBe(
  ...
  it("get rejects more than one section selector (exit 2)", async () => {
    ...
    expect(result.exitCode).toBe(2);
    expect(result.stderr).toContain("one workflow section");
  });
  ...
  it("edit exits 1 with stale_workflow_definition on a stale expectedRevision", async () => {
    ...
    expect(result.exitCode).toBe(1);
    const envelope = JSON.parse(result.stdout);
    expect(envelope.error.details.serverCode).toBe("stale_workflow_definition");
  });
  ...
  it("edit exits 1 with a locator issue for a semantic rejection", async () => {
    ...
    expect(result.exitCode).toBe(1);
    const envelope = JSON.parse(result.stdout);
    expect(envelope.error.details.serverCode).toBe("invalid_edit");
    expect(envelope.error.details.serverIssues[0].path).toBe("operations[0]");
  });

  it("edit --dry-run reports the outcome without persisting", async () => {
    const ops = "/tmp/ops.json";
    const result = await runCcWithHost(
      ["workflow", "edit-preview", "wf-1", "--file", ops],
      env,
      routeHost(null, {
        [ops]: JSON.stringify({
          expectedRevision: 3,
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("Preview: 1 operations");
  });
});
...
describe("cctl workflow author flow against the real create-path validation", () => {
  ...
  it("create posts through the real create-path validation and prints the review-first hint after the advisory", async () => {
    ...
    expect(result.stdout).toContain("cctl workflow get");
    expect(result.stdout).toContain(
  ...
  it("create preserves review-first guidance as one JSON hint", async () => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      payload: {
        data: { item: { id: "wf-new" }, reviewStatus: { state: "unreviewed" } },
      },
      hint: expect.stringContaining("cctl workflow get"),
    });
...
describe("cctl workflow validate assignment error contract (R13.1)", () => {
  ...
  const LOCATED_LINE = /^issue: CC_INPUT_ISSUE: /;
  ...
  describe("malformed values cannot forge a located line", () => {
    ...
    it("keeps duplicate newline-bearing assignment ids to one line each", async () => {
      ...
      for (const line of lines) {
        ...
        expect(line).toMatch(
          /definition\.executionContexts\.1 \(context-implement\)\.contextValidator\.assignments\.\d+ \(evil\\\\n/,
        );
...
describe("cctl workflow assignment provenance (R13.1)", () => {
  it("workflow get shows references — tier:id, strategy, runtime, no revision", async () => {
    ...
    const staffing = result.stdout
      .split("\n")
      .filter((line) => line.startsWith("Staffing "));
...
describe("cctl workflow output-schema read paths (R7.2)", () => {
  ...
  it("workflow get summarizes a declared outputSchema without printing it", async () => {
    ...
    expect(rows.find((row) => row.includes("context-verify"))).toContain(
      'output {"type":"object","fieldCount":2}',
    );
  ...
  it("workflow live get --outputs reports a declared-but-unproduced context as pending", async () => {
    ...
    expect(result.exitCode).toBe(0);
    const envelope = JSON.parse(result.stdout);
    const pending = envelope.payload.data.outputs.find(
  ...
  it("workflow live get --outputs omits contexts that declare no schema", async () => {
    ...
    const envelope = JSON.parse(result.stdout);
    expect(
      envelope.payload.data.outputs.map(
        (entry: { contextId: string }) => entry.contextId,
      ),
...
describe("cctl workflow live get — D4 read surfaces (R13.2)", () => {
  ...
  function routedExecution(): GraphWorkflowExecution {
    ...
    return {
      ...
      contextStates: {
        ...
        "context-plan": {
          ...requiredFixture(execution.contextStates["context-plan"]),
          status: "completed",
        },
        "context-implement": {
          ...requiredFixture(execution.contextStates["context-implement"]),
          status: "skipped",
          skipReason: {
  ...
  function loopExecution(): GraphWorkflowExecution {
    ...
    if (!plan || !implement || !verify)
      throw new Error("The loop fixture requires three contexts.");
    const execution = createWorkflowExecution({
      status: "running",
      workingDefinition: {
        ...base,
    ...
    return {
      ...
      contextStates: {
        ...
        "loop-a__p2__context-verify": {
          ...requiredFixture(execution.contextStates["context-verify"]),
...
describe("the generic clear/release pair is gone, not aliased", () => {
  ...
  it("no surviving surface offers a release verb as a remedy", () => {
    ...
    for (const source of [
      "src/cli/commands/workflow/definitions.ts",
      "src/cli/commands/spec/native-write-definitions.ts",
