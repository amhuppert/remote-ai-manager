...
import { usageFailure, type CliResult } from "./shared";
...
export interface DispatchGroupInput {
  /** The group's full command path, e.g. ["dev"] or ["workflow", "task"]. */
  group: string[];
   ...
   * `fixture session`) pass "verb" to preserve their exact text.
   */
  noun?: "subcommand" | "verb";
}
 ...
 * registry (missing or extra verb) is a wiring defect and throws.
 */
export async function dispatchGroup(
  ...
  if (missing.length > 0 || extra.length > 0) {
    throw new Error(
      `dispatchGroup(${groupKey}): handler map disagrees with the registry — ` +
        `missing [${missing.join(", ")}], extra [${extra.join(", ")}]`,
    );
  }

  const verbList = renderVerbList(verbs);
  const verb = rest[0];
  if (verb === undefined) {
    return usageFailure(`${groupKey} requires a ${noun}: ${verbList}`, json);
  }
  const handler = handlers[verb];
  if (!handler) {
    return usageFailure(
      `unknown ${groupKey} ${noun} "${verb}"; ${groupKey} ${noun}s: ${verbList}`,
      json,
    );
  }
