---
name: nextjs-mcp
description: This skill should be used when diagnosing application errors via the dev server, checking runtime or build errors, inspecting page routes, querying server actions, accessing dev server logs, verifying UI visually, debugging browser-side behavior, or profiling page performance. Relevant trigger phrases include "check for errors", "get page info", "inspect this route", "take a screenshot", "debug the page", "check console logs", "check network requests", "what's wrong with my app", "profile performance", "use next-devtools", "use chrome devtools", or "search next.js docs".
---
