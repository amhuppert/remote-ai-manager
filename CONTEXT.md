...
- **Route resolver (adapter)** — a domain function that produces a
  ...
  which is session-only by spec non-goal. `persistPendingPromptText`
  (`src/lib/prompt/route-handlers.ts`) is the same shape for the conversation
  DRAFT: both adapters resolve into it, and it names the store key
  `storeSessionName` so the sentinel is materialized at the write and never
  bound to a name a log line could pick up.
